...
pub async fn dispatch(cli: Cli, agent_config: &config::AppConfig) -> Result<()> {
    match cli.command {
        Some(Commands::Scan { target, profile }) => {
            run_scan(agent_config, &target, &profile).await?;
        }
...
pub async fn run_scan_with_prompt(
    ...
    if config.tui.enabled {
        ...
        stream::consume_agent_stream(Box::pin(s), executed_tools, &mut agent, rx, start_time)
            .await?;
    }
...
pub async fn run_session_cmd(config: &config::AppConfig, cmd: &str) -> Result<()> {
    ...
    match handle_command(cmd, &mut agent, &mission_events).await {
        ...
        CommandAction::WipeSession => {
            ...
            crate::ui::print_banner(&config.agent.name, &config.agent.version);
            println!(
...
pub async fn run_scan(config: &config::AppConfig, target: &str, profile: &str) -> Result<()> {
    ...
    if config.tui.enabled {
        ...
        interactive::run_interactive_with_context(config, agent, mission_targets).await?;
    }
