...
impl BubblewrapSandbox {
    ...
    fn build_command(
        ...
        // Set internal PATH to include all discovered tool directories + standard system paths
        let mut path_dirs: Vec<String> = self
            .tool_paths
            .iter()
            .filter(|p| !p.is_empty())
            .cloned()
        ...
        // Ensure standard paths are always available for basic tool functionality
        ...
        if let Some(proxy_url) = proxy_url_opt {
            ...
            // Determine the internal sandbox path for proxychains config
            let ctx = crate::config::SystemContext::sense();
            let internal_proxy_config = ctx
                .join_config("proxychains.conf")
                .to_string_lossy()
                .to_string();
