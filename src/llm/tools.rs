...
pub struct ExecuteToolBridge {
    ...
    history_tx: Option<mpsc::UnboundedSender<crate::core::agent::HistoryEvent>>,
}
...
pub struct BridgeContext {
    ...
    pub history_tx: Option<mpsc::UnboundedSender<crate::core::agent::HistoryEvent>>,
}
...
impl Tool for ExecuteToolBridge {
    ...
    async fn call(&self, mut args: Self::Args) -> Result<Self::Output, Self::Error> {
        // --- Tactical Auto-Correction ---
        // If binary is an MCP server name (no slash), and first arg is a tool, rewrite.
        // This handles LLM hallucinations where it forgets the server/tool format.
        if !args.binary.contains('/') {
            let cm = self.clients.read().await;
            if cm.list_client_names().contains(&args.binary) {
                if let Some(tool_candidate) = args.args.first() {
                    let all_tools = cm.list_all_tools().await;
                    if let Some(tools) = all_tools.get(&args.binary) {
                        if tools
                            .iter()
                            .any(|t| t["name"].as_str() == Some(tool_candidate))
                        {
                            let server = args.binary.clone();
                            let tool = tool_candidate.clone();
                            tracing::info!(
                                server = %server,
                                tool = %tool,
                                "Tactical: Auto-correcting MCP tool call (missing slash)"
                            );
                            args.binary = format!("{}/{}", server, tool);
                            args.args.remove(0);
                        }
                    }
                }
        ...
        let full_cmd = format!("{} {}", args.binary, args_joined);

        // Step 1: Global Cache Check
        if let Some(res) = self.executor.check_cache(&full_cmd) {
            return Ok(res.stdout);
        }

        // Step 2: Global Redundancy Check
        self.executor
            .check_redundancy(&full_cmd)
        ...
        if let Some((srv_name, tool_name)) = args.binary.split_once('/') {
            ...
            match cm.call_external_tool(srv_name, tool_name, args_val).await {
                Ok(res) => {
                    ...
                    let res_str =
                        serde_json::to_string_pretty(&res).unwrap_or_else(|_| "null".into());

                    // Record completion and cache
                    self.executor.record_completion(&full_cmd, true);
                    self.executor.update_cache(
                        &full_cmd,
                        crate::mcp::execute::ExecutionResult {
                            command: full_cmd.clone(),
                            stdout: res_str.clone(),
                            stderr: String::new(),
                            exit_code: 0,
                            duration_ms: 0,
                            success: true,
                    ...
                    // Persist to history
                    if let Some(ref htx) = self.history_tx {
                        let _ = htx.send(crate::core::agent::HistoryEvent::ToolResult {
                ...
                Err(e) => {
                    self.executor.record_completion(&full_cmd, false);
                    if let Some(ref tx) = self.event_tx {
                        let _ = tx.send(TuiEvent::ToolFinished {
                            tool: format!("[{}] {}", tool_type_tag, args.binary),
                            success: false,
        ...
        let binary_path = {
            let d = self.discovery.read().await;
            d.get(&args.binary)
                .map(|t| t.path.clone())
            ...
            .await
        {
            Ok(result) => {
                ...
                // Record completion and cache (already handled in executor.execute, but we need history persistence)
                if let Some(ref htx) = self.history_tx {
                    let _ = htx.send(crate::core::agent::HistoryEvent::ToolResult {
                        name: args.binary.clone(),
...
pub struct ExecuteBatchBridge {
    ...
    discovery: Arc<RwLock<ToolDiscovery>>,
    clients: Arc<RwLock<crate::mcp::client::McpClientManager>>,
    _executed_tools: Arc<Mutex<Vec<(String, String)>>>,
    missing_tools: Arc<Mutex<HashSet<String>>>,
    workspace_path: std::path::PathBuf,
    event_tx: Option<mpsc::UnboundedSender<TuiEvent>>,
    builtin_registry: Arc<crate::builtin_tools::registry::BuiltinRegistry>,
...
type ToolFuture<'a> =
    std::pin::Pin<Box<dyn futures::Future<Output = Result<String, ToolError>> + Send + 'a>>;
...
impl ExecuteBatchBridge {
    pub fn new(ctx: BridgeContext) -> Self {
        Self {
            ...
            discovery: ctx.discovery,
            clients: ctx.clients,
            _executed_tools: ctx.executed_tools,
            missing_tools: ctx.missing_tools,
            workspace_path: ctx.workspace_path,
            event_tx: ctx.event_tx,
            builtin_registry: ctx.builtin_registry,
...
impl Tool for ExecuteBatchBridge {
    ...
    async fn call(&self, args: Self::Args) -> Result<Self::Output, Self::Error> {
        ...
        let mut futures: Vec<ToolFuture<'_>> = Vec::new();
        ...
        for cmd in args.commands {
            ...
            let full_cmd = format!("{} {}", binary_name, args_joined);

            // Step 1: Global Cache Check
            if let Some(res) = self.executor.check_cache(&full_cmd) {
                let cached_out = res.stdout.clone();
                futures.push(Box::pin(async move { Ok::<String, ToolError>(cached_out) }));
                continue;
            }

            // Step 2: Global Redundancy Check
            if let Err(e) = self.executor.check_redundancy(&full_cmd) {
                let err_msg = e.to_string();
                futures.push(Box::pin(async move { Err(ToolError(err_msg)) }));
                continue;
            }

            // Step 3: Global Execution Tracker
            self.executor.record_completion(&full_cmd, false); // Mark as in-flight
            executed_batch_info.push(binary_name.clone());

            let executor = self.executor.clone();
            let clients = self.clients.clone();
            let event_tx = self.event_tx.clone();
            let htx = self.history_tx.clone();
            ...
            let q = if is_search_batch {
                structured_args
                    .as_ref()
                    .and_then(|v| {
                        v.get("query")
                            .and_then(|vq| vq.as_str())
                            .map(|s| s.to_string())
                ...
                if let Some((srv_name, tool_name)) = binary_name.split_once('/') {
                    ...
                    match res {
                        Ok(val) => {
                            ...
                            // Record completion and cache
                            executor.record_completion(&full_cmd, true);
                            executor.update_cache(&full_cmd, crate::mcp::execute::ExecutionResult {
                                command: full_cmd.clone(),
                                stdout: out.clone(),
                                stderr: String::new(),
                                exit_code: 0,
                            ...
                            // Persist to history
                            if let Some(ref htx_tx) = htx {
                                let _ = htx_tx.send(crate::core::agent::HistoryEvent::ToolResult {
                        ...
                        Err(e) => {
                            executor.record_completion(&full_cmd, false);
                            Err(ToolError(format!("External tool {} failed: {}", binary_name, e)))
                        }
                    }
                    ...
                    match res {
                        ...
                        Err(e) => {
                            executor.record_completion(&full_cmd, false);
                            Err(ToolError(format!("Native tool {} failed: {}", binary_name, e)))
                        }
                    }
                } else {
                    // Resolve local binary
                    let binary_path = {
                        let d: tokio::sync::RwLockReadGuard<'_, ToolDiscovery> = self.discovery.read().await;
                    ...
                    match executor.execute(&binary_path, &args_refs, &self.workspace_path, Some(callback)).await {
                         Ok(res) => {
                            if is_search_batch {
                        ...
                        Err(e) => {
                            ...
                            if matches!(e, crate::mcp::execute::ExecuteError::MissingBinary(_)) {
                                let mut mt: tokio::sync::MutexGuard<'_, HashSet<String>> = self.missing_tools.lock().await;
                                mt.insert(binary_name.clone());
                            }
                            Err(ToolError(e.to_string()))
                        }
                    }
