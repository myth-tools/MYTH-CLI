...
# Neural Pulse-Bar Renderer (v3.5)
render_bar() {
    local percent="$1"
    local bar_size=10
...
# The Ticker v3.5: Compact Single-Line High-Density Telemetry
hud_ticker() {
    local start_time="$1"
    local est_seconds="$2"
    local objective="$3"
    ...
    # Truncate objective to fit
    local cols
    cols=$(tput cols 2>/dev/null || echo 80)
    local max_obj_len=$((cols / 3))
    if [ "${#objective}" -gt "$max_obj_len" ]; then
        objective="${objective:0:$((max_obj_len-3))}..."
    fi
    ...
    while true; do
        ...
        local rem_abs=$(( remaining < 0 ? -remaining : remaining ))
        local t_str
        if [ "$rem_abs" -ge 60 ]; then
            t_str=$(printf "%dm %ds" $((rem_abs / 60)) $((rem_abs % 60)))
        else
            t_str="${rem_abs}s"
        fi

        local status_color="\033[37m"
        ...
        if [ "$HUD_TERM_SUPPORT" = true ]; then
            # SINGLE LINE UPDATE
            echo -ne "\r \033[34m${frm}\033[0m  ${BOLD}${objective}${NC}  \033[0;90m${bar} \033[36m${pct}%\033[0;90m | \033[35m${speed_str}\033[0;90m | ${status_color}${t_str}${NC}\033[K" >&3
        fi
    done
}

# Premium HUD: Start a mission step with a single-line high-density ticker
step_start() {
    local objective="$1"
    local base_est="$2"
    local multiplier
    ...
    if [ "$HUD_TERM_SUPPORT" = true ]; then
        ...
        local start_t
        start_t=$(date +%s)
        hud_ticker "$start_t" "$HUD_EST_SECONDS" "$objective" &
        HUD_TICKER_PID=$!
        disown "$HUD_TICKER_PID" 2>/dev/null || true
    else
        echo -ne " ${BLUE}⠿${NC}  ${BOLD}${objective}${NC}..." >&3
    fi
}

# Premium HUD: Banish the Ticker and mark success (Single Line)
step_done() {
    local message="${1:-$HUD_ACTIVE_STEP complete.}"
    if [ "$HUD_TERM_SUPPORT" = true ]; then
        if [ -n "$HUD_TICKER_PID" ]; then kill -9 "$HUD_TICKER_PID" 2>/dev/null || true; fi
        # Overwrite the ticker line with final status
        echo -e "\r ${GREEN}[ OK ]${NC} ${BOLD}${message}${NC}\033[K" >&3
    else
...
step_fail() {
    ...
    if [ "$HUD_TERM_SUPPORT" = true ]; then
        if [ -n "$HUD_TICKER_PID" ]; then kill -9 "$HUD_TICKER_PID" 2>/dev/null || true; fi
        echo -e "\r ${RED}[ FAIL ]${NC} ${BOLD}${err_msg}${NC}\033[K" >&3
    else
...
# Industry Grade: Non-fatal failure
step_warn() {
    local warn_msg="${1:-$HUD_ACTIVE_STEP encountered a non-fatal error.}"
    if [ "$HUD_TERM_SUPPORT" = true ]; then
        if [ -n "$HUD_TICKER_PID" ]; then kill -9 "$HUD_TICKER_PID" 2>/dev/null || true; fi
        echo -e "\r ${YELLOW}[ WARN ]${NC} ${BOLD}${warn_msg}${NC}\033[K" >&3
    else
    ...
    if [ -z "$GH_BINARY_NAME" ]; then
        ...
        if command -v curl &>/dev/null; then
            LATEST_TAG=$(curl -fsSL \
                ...
                2>/dev/null | grep '"tag_name"' | head -1 | sed -E 's/.*"tag_name": *"([^"]+)".*/\1/' || echo "")
        fi
        ...
        if [ -z "$LATEST_TAG" ]; then
            ...
            for attempt in 1 2 3; do
                # Capture stderr to log file to avoid mangling HUD visuals
                if curl -fsSL --progress-bar "$GH_BINARY_URL" -o "$TEMP_BINARY" 2>>"$LOG_FILE"; then
            ...
            if [ "$DOWNLOAD_SUCCESS" = true ] && [ -s "$TEMP_BINARY" ]; then
                ...
                if curl -fsSL "${PAGES_URL}/SHA256SUMS" -o "$TEMP_SUMS" 2>/dev/null; then
                    ...
                    if grep -q "$GH_BINARY_NAME" "$TEMP_SUMS"; then
                        ...
                        EXPECTED_SHA=$(grep "$GH_BINARY_NAME" "$TEMP_SUMS" | awk '{print $1}' || echo "")
                        ACTUAL_SHA=$(sha256sum "$TEMP_BINARY" | awk '{print $1}' || echo "error")
