    ...
    async fn call(&self, args: Self::Args) -> Result<Self::Output, Self::Error> {
        ...
        if let Some(ref tx) = self.event_tx {
            let _ = tx.send(TuiEvent::ToolStarted {
                ...
                command: format!("{} {}", args.binary, args_joined),
            });
        } else {
        ...
        if let Some((srv_name, tool_name)) = args.binary.split_once('/') {
            ...
            let args_val = args.structured_args.clone().unwrap_or_else(|| {
                serde_json::to_value(&args.args).unwrap_or(serde_json::Value::Array(vec![]))
            });
...
impl Tool for DiscoverToolsBridge {
    ...
    async fn call(&self, args: Self::Args) -> Result<Self::Output, Self::Error> {
        ...
        let mut mcp_entries: Vec<serde_json::Value> = Vec::new();
        {
            ...
            let include_mcp = cat_lower
                .as_ref()
                .is_none_or(|c| c.contains("mcp") || c.contains("external") || c.contains("all"));
        ...
        let mut native_entries: Vec<serde_json::Value> = Vec::new();
        {
            ...
            let include_native = cat_lower.as_ref().is_none_or(|c| {
                c.contains("native")
                    || c.contains("utility")
                    || c.contains("builtin")
                    || c.contains("all")
...
impl Tool for ExecuteBatchBridge {
    ...
    async fn call(&self, args: Self::Args) -> Result<Self::Output, Self::Error> {
                ...
                if let Some((srv_name, tool_name)) = binary_name.split_once('/') {
                    ...
                    if let Some(ref tx) = event_tx {
                        let _ = tx.send(TuiEvent::ToolStarted {
                            ...
                            command: format!("{} {}", binary_name, args_vec.join(" "))
                        });
                    }

                    let cm = clients.read().await;
                    let args_val = structured_args.clone()
                        .unwrap_or_else(|| serde_json::to_value(&args_vec).unwrap_or(serde_json::Value::Array(vec![])));
                    ...
                    if let Some(ref tx) = event_tx {
                        let _ = tx.send(TuiEvent::ToolStarted {
                            tool: binary_name.clone(),
                            command: format!("{} (native)", binary_name)
                        });
                    }
                    ...
                    match res {
                        Ok(val) => Ok(serde_json::to_string_pretty(&val).unwrap_or_else(|_| "null".into())),
        ...
        for (i, res) in results.into_iter().enumerate() {
            let tool_name = &executed_batch_info[i];
