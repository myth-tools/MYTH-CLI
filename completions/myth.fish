...
complete -c myth -n "__fish_myth_needs_command" -s l -l log-level -d 'Override log level (trace, debug, info, warn, error)' -r
...
complete -c myth -n "__fish_myth_needs_command" -f -a "version" -d 'Display the current neural core version version'
...
complete -c myth -n "__fish_myth_using_subcommand help; and not __fish_seen_subcommand_from scan stealth osint vuln tools target chat config profile check mcp vitals findings graph history report sync burn wipe clear depth inspect completions usage version subdomains master help" -f -a "version" -d 'Display the current neural core version version'
