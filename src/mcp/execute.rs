...
struct RedundancyMonitor {
    ...
    pending_commands: std::collections::HashSet<String>, // commands currently in flight
}
...
impl ToolExecutor {
    ...
    /// Check if a command is redundant or exceeds turn limits.
    pub fn check_redundancy(&self, full_cmd: &str) -> Result<(), ExecuteError> {
        ...
        // Check if identical command is already in flight (Parallel Swarm Protection)
        if monitor.pending_commands.contains(full_cmd) {
            return Err(ExecuteError::RedundantCommand(
                format!("{} is already executing in parallel. Wait for the result instead of launching redundant swarms.", full_cmd)
            ));
        }
        ...
        // Mark as pending and increment count
        monitor.pending_commands.insert(full_cmd.to_string());
        monitor.execution_count += 1;
        Ok(())
    }

    /// Record the completion of a tool command.
    pub fn record_completion(&self, full_cmd: &str, success: bool) {
        let mut monitor = self.redundancy_monitor.lock().unwrap();
        monitor.pending_commands.remove(full_cmd);
        monitor.history.push_front((full_cmd.to_string(), success));
        if monitor.history.len() > 16 {
            monitor.history.pop_back();
        }
    }

    /// Check the output cache for a command.
    pub fn check_cache(&self, full_cmd: &str) -> Option<ExecutionResult> {
        if let Some(cached) = self.output_cache.get(full_cmd) {
            let (res, ts) = cached.value();
            // Tactical: 30s Dampening for identical rapid-fire calls
            if ts.elapsed() < std::time::Duration::from_secs(30) {
                tracing::info!(cmd = %full_cmd, "Neural Cache HIT: Dampening redundant swarm");
    ...
    pub fn update_cache(&self, full_cmd: &str, result: ExecutionResult) {
        self.output_cache
            .insert(full_cmd.to_string(), (result, std::time::Instant::now()));
    }
    ...
    pub async fn execute(
        ...
        if let Some(res) = self.check_cache(&full_cmd) {
            return Ok(res);
        }

        // Step 1.5: Redundancy Check (Crush hallucinations)
        self.check_redundancy(&full_cmd)?;
        ...
        self.record_completion(&full_cmd, execution_result.success);

        // Update output cache
        self.update_cache(&full_cmd, execution_result.clone());
    ...
    pub fn reset_redundancy_monitor(&self) {
        ...
        monitor.pending_commands.clear();
