...
pub fn get_man_page(topic: &str) -> Option<String> {
    match topic.to_lowercase().as_str() {
        ...
        "browser" | "lightpanda" => Some(format!(
            "\n{title}\n\n\
             {desc}\n\n\
             {details}\n\
             The PRIMARY high-performance browser engine for MYTH. \n\
             Built in Zig for ultra-fast, zero-latency JS execution. \n\
             Used for deep tactical reconnaissance and stealth web data acquisition.\n\n\
             {features_label}\n\
             - 11x faster than traditional Chromium-based solutions.\n\
             - 9x less memory footprint.\n\
             - Zero-Touch: Automatically provisioned on first reconnaissance.\n\
             - Native: Executed directly via Stdio for maximum throughput.\n\n\
             FALLBACK ENGINES:\n\
             - Playwright: Legacy fallback for ultra-complex 100% Chromium CDP cases.\n",
            title = "MAN PAGE: NEURAL ASSET // BROWSER".bold().reversed().cyan(),
            desc = "Advanced AI-native headless browser for tactical web ops.".italic(),
            details = "DETAILS:".dimmed(),
            features_label = "KEY FEATURES:".dimmed(),
        )),
        _ => None,
    }
}
...
pub fn get_usage() -> String {
    format!(
        "\n{title}\n\n\
         ...
         {sys_group}\n\
         - /man <topic>    Deep-dive (try '/man browser')\n\
         - /typography     Manage tactical fonts\n\
