...
impl McpServer {
    ...
    pub async fn new(
        ...
        let init_handle = tokio::spawn(async move {
            ...
            Self::sync_clients_arc(clients_init.clone(), config_init.mcp.mcp_servers.clone()).await;
    ...
    pub async fn sync_clients(&self, config: &AppConfig) {
        ...
        let servers = config.mcp.mcp_servers.clone();
        tokio::spawn(async move {
            Self::sync_clients_arc(clients, servers).await;
        });
    }
    ...
    pub async fn sync_clients_arc(
        ...
        // 4. Insert new clients and warm the tool cache non-blocking
        while let Some((name, result)) = add_futures.next().await {
            match result {
                Ok(client) => {
                    // PHOENIX: Pre-fetch tools over the network WITHOUT holding any UI-blocking locks
