...
use crate::config::context::SystemContext;
...
impl AppConfig {
    ...
    pub fn config_dir() -> PathBuf {
        ...
        let system_dir = ctx.join_config("myth");

        if system_dir.is_dir() {
            return system_dir;
        }

        // Default to user XDG config dir (e.g. ~/.config/myth)
        // We do NOT use hardcoded fallback for user data.
        dirs::config_dir()
            .map(|p| p.join("myth"))
            .unwrap_or_else(|| {
                // Absolute fallback if no HOME or CONFIG_DIR
                PathBuf::from(".myth")
            })
    }
}
    ...
    pub fn merge_with_default(&mut self, default: &CustomMcpServer) -> bool {
        ...
        match (self, default) {
            (CustomMcpServer::Local(ref mut cur), CustomMcpServer::Local(def)) => {
                ...
                if cur.working_dir != def.working_dir {
                    cur.working_dir = def.working_dir.clone();
                    changed = true;
                }
                if cur.allowed_tools != def.allowed_tools {
                    cur.allowed_tools = def.allowed_tools.clone();
                    changed = true;
                }
                ...
                if cur.timeout != def.timeout {
                    cur.timeout = def.timeout;
                    changed = true;
                }
                if cur.install_script != def.install_script {
                    cur.install_script = def.install_script.clone();
                    changed = true;
                }
...
pub struct LocalMcpConfig {
    ...
    /// \[OPTIONAL\] Shell script to auto-install the binary if missing from PATH.
...
fn default_tool_paths() -> Vec<String> {
    ...
    // Termux Support: prioritize $PREFIX/bin
    if let Ok(prefix) = std::env::var("PREFIX") {
        if !prefix.is_empty() {
            paths.insert(0, format!("{}/bin", prefix));
            paths.insert(1, format!("{}/bin/applets", prefix));
        }
    }
    // Include user-local bin for non-root installs on any distro
    if let Ok(home) = std::env::var("HOME") {
        let local_bin = format!("{}/.local/bin", home);
        paths.push(local_bin);
    }
    // Termux (Android): $PREFIX/bin takes priority
    if let Ok(prefix) = std::env::var("PREFIX") {
        if !prefix.is_empty() {
            paths.insert(0, format!("{}/bin", prefix));
        }
...
pub struct SandboxConfig {
    ...
    #[serde(default = "default_read_only_paths")]
    pub read_only_paths: Vec<String>,
    #[serde(default = "default_writable_tmpfs")]
...
fn default_bwrap_path() -> String {
    ...
    let mut search_paths = vec![
        ...
        "/usr/local/bin/bwrap".to_string(),
        "/bin/bwrap".to_string(),
        "/usr/sbin/bwrap".to_string(),
    ];

    if ctx.is_termux {
        if let Some(prefix) = &ctx.prefix {
            search_paths.insert(0, format!("{}/bin/bwrap", prefix));
        }
    }
    ...
    // Final fallback: hope it's in PATH or at the most common location
    if ctx.is_termux {
        if let Some(prefix) = &ctx.prefix {
            return format!("{}/bin/bwrap", prefix);
        }
    }
...
fn default_read_only_paths() -> Vec<String> {
    let ctx = SystemContext::sense();

    let paths = if ctx.is_termux {
        vec![
            ctx.join_config("resolv.conf").to_string_lossy().to_string(),
            ctx.join_config("tls").to_string_lossy().to_string(),
            ctx.join_config("fonts").to_string_lossy().to_string(),
            ctx.join_config("ca-certificates")
                .to_string_lossy()
                .to_string(),
        ]
    } else {
    ...
    // Filter to existing paths to prevent bwrap "no such file" errors
    paths
        .into_iter()
...
fn default_writable_tmpfs() -> Vec<String> {
    let mut paths = vec![
        "/tmp".to_string(),
        "/var/tmp".to_string(),
        "/run/user".to_string(),
...
pub struct TuiConfig {
    ...
    pub last_left_width: u16,
    #[serde(default = "default_right_width")]
    pub last_right_width: u16,
    #[serde(default)]
...
pub struct PipelineConfig {
    ...
    pub enabled: bool,
    #[serde(default)]
    pub evaluator: AgentSlotConfig,
    #[serde(default)]
    pub gatherer: GathererSlotConfig,
    #[serde(default)]
    pub aggregator: AgentSlotConfig,
    #[serde(default)]
    pub planner: AgentSlotConfig,
    #[serde(default)]
    pub executor: ExecutorSlotConfig,
    #[serde(default)]
    pub validator: ValidatorSlotConfig,
    #[serde(default)]
    pub reporter: AgentSlotConfig,
    #[serde(default)]
    pub human_review: HumanReviewSlotConfig,
}
...
pub struct AgentSlotConfig {
    ...
    #[serde(default = "default_slot_temperature")]
    pub temperature: f32,
    #[serde(default = "default_slot_max_tokens")]
    pub max_tokens: u32,
    #[serde(default)]
...
#[derive(Debug, Clone, Serialize, Deserialize, Default)]
pub struct ValidatorSlotConfig {
    #[serde(default = "default_true")]
    pub enabled: bool,
    pub model: Option<String>,
    #[serde(default = "default_slot_temperature")]
...
#[derive(Debug, Clone, Serialize, Deserialize, Default)]
pub struct HumanReviewSlotConfig {
    #[serde(default = "default_true")]
    pub enabled: bool,
    #[serde(default)]
    pub description: String,
...
fn default_slot_temperature() -> f32 {
    0.3
}
fn default_slot_max_tokens() -> u32 {
    16384
}
...
#[derive(Debug, Clone, Serialize, Deserialize, Default)]
pub struct UserTuiConfig {
    pub enabled: Option<bool>,
    pub font: Option<String>,
    pub simulated_mode: Option<bool>,
    pub last_left_width: Option<u16>,
    pub last_right_width: Option<u16>,
}
...
impl AppConfig {
    ...
    fn find_user_config() -> Option<PathBuf> {
        ...
        let ctx = SystemContext::sense();
        candidates.push(ctx.join_config("myth/user.yaml"));
    ...
    fn find_mcp_config() -> Option<PathBuf> {
        ...
        let ctx = SystemContext::sense();
        candidates.push(ctx.join_config("myth/mcp.json"));
    ...
    fn apply_user_config(&mut self, user: &UserConfig) {
        ...
        if let Some(ref provider) = user.provider {
            if !provider.nvidia_nim_api_key.is_empty() {
                self.llm.nvidia_nim_api_key = provider.nvidia_nim_api_key.clone();
            }
        ...
        // TUI overrides
        if let Some(ref tui) = user.tui {
            if let Some(enabled) = tui.enabled {
                self.tui.enabled = enabled;
            }
            if let Some(ref font) = tui.font {
                self.tui.font = font.clone();
            }
            if let Some(lw) = tui.last_left_width {
                self.tui.last_left_width = lw;
            }
            if let Some(rw) = tui.last_right_width {
                self.tui.last_right_width = rw;
            }
        }
    }
