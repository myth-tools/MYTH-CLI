...
pub struct McpClientManager {
    ...
    tool_cache: dashmap::DashMap<String, Vec<Value>>,
}
...
impl McpClientManager {
    pub fn new() -> Self {
        Self {
            clients: HashMap::new(),
            tool_cache: dashmap::DashMap::new(),
        }
    }
    ...
    pub fn insert_tool_cache(&mut self, name: &str, tools: Vec<Value>) {
        self.tool_cache.insert(name.to_string(), tools);
    }
    ...
    pub async fn list_all_tools(&self) -> HashMap<String, Vec<Value>> {
            ...
            && self.clients.keys().all(|k| self.tool_cache.contains_key(k))
        {
            ...
            for item in self.tool_cache.iter() {
                all_tools.insert(item.key().clone(), item.value().clone());
            }
            return all_tools;
        }
        ...
        for (name, res) in results {
            match res {
                Ok(Ok(tools)) => {
                    self.tool_cache.insert(name.clone(), tools.clone());
...
impl SseMcpClient {
    pub fn new(id: String, config: RemoteMcpConfig) -> Self {
        ...
        for (k, v) in &config.headers {
            let effective_val = if v.is_empty() {
                std::env::var(k).unwrap_or_default()
            } else {
                v.clone()
            };

            if !effective_val.is_empty() {
                if let Ok(hv) = reqwest::header::HeaderValue::from_str(&effective_val) {
    ...
    async fn send_request(&self, method: &str, params: Value) -> Result<Value, McpClientError> {
        ...
        let timeout_secs = if method == "initialize" {
            300 // PHOENIX: Allow up to 5 minutes for first-time binary/package downloads
        } else {
            self.config.timeout
        };
...
impl McpClient for StdioMcpClient {
    async fn connect(&mut self) -> Result<(), McpClientError> {
        ...
        // --- Industry Grade Environment Sanitization ---
        // Resolve the correct local binary directory:
        //   $PREFIX/bin  on Termux (Android)
        //   ~/.local/bin on standard Linux
        let home = std::env::var("HOME").unwrap_or_else(|_| "/root".into());
        let install_bin_dir = if let Ok(prefix) = std::env::var("PREFIX") {
        ...
        if !current_path.contains(&install_bin_dir) {
            command.env("PATH", format!("{}:{}", install_bin_dir, current_path));
        }

        // --- Auto-Provisioning Block ---
        // If binary is missing and an install_script is provided, attempt autonomous installation.
        if let Some(ref script) = self.config.install_script {
            // Check if binary is in the system PATH or our install_bin_dir
            let binary_path = tokio::process::Command::new("which")
                .env("PATH", format!("{}:{}", install_bin_dir, current_path))
            ...
            if binary_path.is_none() {
                // --- Concurrency Synchronization (Elite Industry Standard) ---
                let lock_dir = format!("{}/.lightpanda.lock", install_bin_dir);
                let lock_path = std::path::Path::new(&lock_dir);

                // If lock exists, wait for it to be released (another engine is provisioning)
                let mut attempts = 0;
                while lock_path.exists() && attempts < 30 {
                    tracing::warn!(
                        server = %self.id,
                        "PHOENIX: Another provisioning engine is active. Waiting for system lock... [{}/30]",
                        attempts
                    );
                ...
                // If binary still missing after lock release (or no lock), provision it
                let binary_check = format!("{}/{}", install_bin_dir, self.config.command);
                if !std::path::Path::new(&binary_check).exists() {
                    tracing::info!(
                        server = %self.id,
                        command = %self.config.command,
                        "PHOENIX: Core asset missing. Executing Level-1 Autonomous Provisioning..."
                    );

                    let mut install_cmd = tokio::process::Command::new("bash");
                    install_cmd.arg("-c").arg(script);

                    match install_cmd.status().await {
                        Ok(status) if status.success() => {
                            // --- Atomic Integrity Check ---
                            // Verify that the provisioned binary is a valid ELF executable (Industry Grade)
                            let binary_path =
                                format!("{}/{}", install_bin_dir, self.config.command);
                            if let Ok(mut file) = std::fs::File::open(&binary_path) {
                                use std::io::Read;
                                let mut buffer = [0; 4];
                                if file.read_exact(&mut buffer).is_ok() {
                                    if &buffer == b"\x7fELF" {
                                        tracing::info!(server = %self.id, "PHOENIX: Asset provisioned and integrity verified.");
                                    } else {
                                        tracing::error!(server = %self.id, "PHOENIX: Integrity failure [INVALID_MAGIC]");
                        ...
                        Ok(status) => {
                            tracing::error!(
                                server = %self.id,
                                "PHOENIX: Provisioning failed [CODE: {:?}]",
                                status.code()
                            );
                        }
                        Err(e) => {
                            tracing::error!(server = %self.id, error = %e, "PHOENIX: Provisioning engine error");
                        }
                    }
                }
            }
        ...
        let server_id_err = self.id.clone();
        let stderr_capture = Arc::new(Mutex::new(Vec::new()));
        let stderr_capture_for_bg = Arc::clone(&stderr_capture);
        tokio::spawn(async move {
            let mut reader = BufReader::new(stderr).lines();
            while let Ok(Some(line)) = reader.next_line().await {
                tracing::debug!(server = %server_id_err, "Stderr: {}", line);
                let mut capture = stderr_capture_for_bg.lock().await;
                capture.push(line);
                if capture.len() > 10 {
                    capture.remove(0);
                }
        ...
        let dispatch = Arc::clone(&self.dispatch);
        let stderr_capture_for_stdout = Arc::clone(&stderr_capture);
        let server_id_stdout = self.id.clone();
        tokio::spawn(async move {
            while let Ok(Some(line)) = reader.next_line().await {
            ...
            // PHOENIX Resilience: If stdout closes, the process likely exited.
            // Notify all pending dispatchers immediately to avoid long timeouts.
            let mut diagnostics = String::new();
            {
                let capture = stderr_capture_for_stdout.lock().await;
                if !capture.is_empty() {
                    diagnostics = format!("\nRecent Stderr:\n{}", capture.join("\n"));
                }
            }
            ...
            let error_msg = if diagnostics.is_empty() {
                "MCP process exited or stdout closed unexpectedly".to_string()
            } else {
                format!("MCP process exited unexpectedly.{}", diagnostics)
            };

            tracing::warn!(server = %server_id_stdout, "PHOENIX: Connection lost, failing {} pending requests", dispatch.len());

            // Collect keys first to avoid deadlocks/iter issues while removing
            let pending_ids: Vec<u64> = dispatch.iter().map(|item| *item.key()).collect();
            for id in pending_ids {
                if let Some((_, tx)) = dispatch.remove(&id) {
                    let _ = tx.send(Err(McpClientError::ConnectionFailed(error_msg.clone())));
                }
            }
        });
