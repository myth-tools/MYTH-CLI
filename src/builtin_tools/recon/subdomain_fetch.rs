// =============================================================================
// ⚡ MYTH SOVEREIGN DISCOVERY ENGINE — v0.1.0-QUANTUM
// =============================================================================
//
// 💎 TECHNICAL SPECIFICATION:
// ─────────────────────────────────────────────────────────────────────────────
// ROLE        : Ultra-High-Performance Subdomain Intelligence & Asset Mapping
// PIPELINE    : 18-Phase Asynchronous Discovery (Streaming Reactive Logic)
// ARCHITECTURE: Zero-Allocation Regex Engine | 2048-Shard Concurrent DashMaps
// ANONYMITY   : Sovereign Tor Resonance | Adaptive Proxy Circuit Breaker
// FIDELITY    : High-Precision Wildcard Fingerprinting | NSEC3 Zone Mapping
// RESOLUTION  : Tier-1 Dynamic Resolver Pool | Tier-2 Cloudflare/Google DoH
// ─────────────────────────────────────────────────────────────────────────────
//
// LICENSING: PROPRIETARY // MISSION-CRITICAL OPS
// =============================================================================
...
static SUBDOMAIN_RE: Lazy<Regex> = Lazy::new(|| {
    ...
    .expect("Industrial-Grade Static Pattern Analysis: SUBDOMAIN_RE compilation failed. Check regex syntax.")
});

static HTML_TITLE_RE: Lazy<Regex> = Lazy::new(|| {
    Regex::new(r"(?i)<title>(.*?)</title>")
        .expect("Industrial-Grade Static Pattern Analysis: HTML_TITLE_RE compilation failed.")
});

static JS_RE: Lazy<Regex> = Lazy::new(|| {
    Regex::new(r#"(?:src|href|url|api|base|endpoint|host|link|target)['"]?\s*[:=]\s*['"]?([^'"]+\.js(?:on)?)"#).expect("Industrial-Grade Pattern Failure: JS_RE")
});

static MAP_RE: Lazy<Regex> = Lazy::new(|| {
    Regex::new(r"//#\s*sourceMappingURL=([^\s]+)")
        .expect("Industrial-Grade Pattern Failure: MAP_RE")
});

static CSRF_RE: Lazy<Regex> = Lazy::new(|| {
    Regex::new(r#"csrfmiddlewaretoken['"]\s*value=['"]([^'"]+)['"]"#)
        .expect("Industrial-Grade Pattern Failure: CSRF_RE")
});

static ANSI_RE: Lazy<Regex> = Lazy::new(|| {
    Regex::new(r"[\u001b\u009b][\[()#;?]*(?:[0-9]{1,4}(?:;[0-9]{0,4})*)?[0-9A-ORZcf-nqry=><]")
        .expect("Industrial-Grade Pattern Failure: ANSI_RE")
});
...
// Finalized Phase 13 Constants
const MAX_BACKOFF_SECS: u64 = 60;
const MAX_CONSECUTIVE_ERRORS: usize = 5;
// const REQUEST_QUEUE_SIZE: usize = 10000;
const PROXY_TEST_TIMEOUT_SECS: u64 = 5;
const HUD_WIDTH: usize = 60;

// ─── TUI LAYOUT ENGINE (Calculation-First Architecture) ───

fn calculate_visual_width(text: &str) -> usize {
    ...
    for c in stripped.chars() {
        if c.is_ascii() {
            ...
            // High-probability emoji/symbol range (forced 2 cells)
            width += 2;
        } else {
            width += 1;
        }
...
fn draw_box_line(icon: &str, label: &str, value: &str, width: usize) -> String {
    let label_part = if label.is_empty() {
        "".to_string()
    } else {
        format!("{}: ", label)
    };
    let content = format!(" {} {}{}", icon, label_part, value);
    let vis_content = ANSI_RE.replace_all(&content, "");
    let vis_width = calculate_visual_width(&vis_content);
...
// Sovereign Verification Targets (Phase 16)
const PROXY_VERIFY_TARGETS: &[&str] = &[
    "http://icanhazip.com",
    "http://ifconfig.me/ip",
...
impl Default for Config {
    fn default() -> Self {
        Self {
            ...
            proxies_file: None,
            use_tor: true,
            tor_address: "socks5h://127.0.0.1:9050".to_string(),
            checkpoints: true,
...
pub type DnsCacheMap = DashMap<String, Option<(Vec<String>, Option<String>)>>;
...
pub struct AppState {
    ...
    pub discovered: Arc<DashMap<String, ()>>, // Consolidated bloom_filter + discovered map
    ...
    pub tried_bloom: Arc<DashMap<u64, ()>>,
    pub word_bloom: Arc<DashMap<u64, ()>>,
    pub dns_cache: Arc<DnsCacheMap>,
    ...
    // PHASE 1: PRE-COMPILED REGEXES (ZERO-LATENCY)
    pub subdomain_regex: Regex,
    pub html_title_regex: Regex,
    pub js_regex: Regex,
    pub map_regex: Regex,
    pub csrf_regex: Regex,
    pub dynamic_subdomain_regex: Arc<DashMap<String, Regex>>,
}
...
impl SovereignUi {
    pub async fn render(state: &Arc<AppState>) {
        ...
        let phase_bar = format!("[{}] {}/18", bar, phase_idx);

        print!("\x1B[s"); // Save cursor position
        println!("\r\n\x1B[1A\x1B[2K"); // Move up and clear line

        let header_content = format!("{} SOVEREIGN INTELLIGENCE", *AGENT_NAME);
        let header_vis = calculate_visual_width(&header_content) + 7;
        let header_pad = (HUD_WIDTH + 4).saturating_sub(header_vis);

        println!(
            "  ┌── {} {}┐",
            header_content.bright_white().bold(),
            "─".repeat(header_pad)
        );
        println!("  {}", draw_box_line("🎯", "Phase", &phase_bar, HUD_WIDTH));
        println!(
            "  {}",
            draw_box_line(
                "📋",
                "Status",
                &phase_name.bright_white().to_string(),
                HUD_WIDTH
            )
        );
        println!("  ├{}┤", "─".repeat(HUD_WIDTH + 2));
        ...
        println!(
            "  {}",
            draw_box_line("📊", "Status", &status_text, HUD_WIDTH)
        );
        ...
        println!("  {}", draw_box_line("🛰️ ", "Recon", &telemetry, HUD_WIDTH));
        let health = format!(
            "{} Tasks | {} Nodes | {} Proxies",
        ...
        println!("  {}", draw_box_line("🌐", "Health", &health, HUD_WIDTH));
        println!(
            "  {}",
            draw_box_line(
                "🕵️ ",
                "Last",
                &last_sub.clone().bright_white().italic().to_string(),
                HUD_WIDTH
            )
        );
        println!("  └{}┘", "─".repeat(HUD_WIDTH + 2));
...
async fn run_fetch_internal(mut config: Config) -> Result<Vec<SubdomainResult>> {
    ...
    if !config.quiet {
        ...
        println!(
            "║ {} — {} ║",
            format!("{} SOVEREIGN DISCOVERY ENGINE", *AGENT_NAME)
                .bold()
                .cyan(),
    ...
    if !state.config.quiet {
        let banner_width = HUD_WIDTH;
        let header_content = format!("{} ELITE SUBDOMAIN FETCHER", *AGENT_NAME);
        let header_vis = calculate_visual_width(&header_content) + 6; // "  ┌── " is 6
        let header_pad = (banner_width + 4).saturating_sub(header_vis);
        ...
        println!(
            "  {}",
            draw_box_line("🔥", "Version", &VERSION, banner_width)
        );
        println!(
            "  {}",
            draw_box_line(
                "🚀",
                "Industry",
                "Elite Asset Discovery [ALFA-SHIELD]",
                banner_width
            )
        );
        println!("  ├{}┤", "─".repeat(banner_width + 2));
        println!(
            "  {}",
            draw_box_line("🎯", "Goal", "~100% Discovery Fidelity", banner_width)
        );
        println!(
            "  {}",
            draw_box_line(
                "📡",
                "Performance",
                &format!("{} parallel threads", state.config.concurrency),
                banner_width
            )
        );
        println!(
            ...
            draw_box_line(
                "🛡️ ",
                "Resolvers",
                &format!("{} dynamic nodes", state.resolvers.read().await.len()),
                banner_width
            )
        );
        println!(
            ...
            draw_box_line(
                "🕵️ ",
                "Stealth",
                "Reactive Resonance Intelligence",
                banner_width
            )
        );
        println!("  └{}┘", "─".repeat(banner_width + 2));
...
async fn initialize_state(config: Config) -> Result<Arc<AppState>> {
    ...
    // ─── Phase 8: Exhaustive Tor Discovery & Verification Pool ───
    let mut final_tor_address = String::from("DIRECT");
    let mut active_client_builder = ClientBuilder::new()
    ...
    if config.use_tor {
        let mut candidate_ports = vec![9050, 9150, 9051];
        if let Some(user_port) = config
            .tor_address
            .split(':')
            .next_back()
            .and_then(|p| p.parse::<u16>().ok())
        {
            if !candidate_ports.contains(&user_port) {
                candidate_ports.insert(0, user_port);
            }
        }
        ...
        for (pass_idx, port) in candidate_ports.iter().enumerate() {
            let port = *port;
            let addr = format!("127.0.0.1:{}", port);
            let proxy_url = format!("socks5h://{}", addr);

            if !config.quiet {
                println!("🔍 Checking port {} for active Tor service...", port);
            }

            // 1. TCP Handshake (Quick check)
            ...
            .is_err()
            {
                // If the first pass across ALL ports fails, try a one-time service rescue
                if pass_idx == candidate_ports.len() - 1 && discovered_port.is_none() {
                    if !config.quiet {
                        println!(
                            "⚠️  Tor ports unreachable. Attempting Autonomous Service Rescue..."
                        );
                    }
                    attempt_tor_service_recovery().await;
                    // Note: We don't recurse here, we just hope the next ports/retries find it
                }
                diagnostic_log.push(format!("Port {}: Connection Refused/Timed out", port));
                continue;
            }

            // ─── Phase 10: Adaptive Tor Bootstrapping ───
            let mut port_verified = false;
            let mut retry_count = 0;
            let max_retries = 3;

            while retry_count <= max_retries {
                let test_client = match ClientBuilder::new()
                    .proxy(Proxy::all(&proxy_url)?)
                    .timeout(Duration::from_secs(20)) // Sovereign patience for bootstrap
                    .build()
                {
                    Ok(c) => c,
                    Err(e) => {
                        diagnostic_log
                            .push(format!("Port {}: Client Construction Failed ({})", port, e));
                        break;
                    }
                };

                let verification_targets = [
                    ("http://138.201.14.212/api/ip", "Tor Project (Raw IP)"),
                    ("http://104.18.23.51", "Icanhazip (Raw IP)"),
                ];

                for (target_url, target_name) in verification_targets {
                    if !config.quiet && retry_count == 0 {
                        println!("  ↳ 🛰️  Verifying {} via port {}...", target_name, port);
                    }

                    let res = tokio::time::timeout(
                        Duration::from_secs(20),
                        test_client.get(target_url).send(),
                    )
                    .await;
                    if let Ok(Ok(_resp)) = res {
                        // ─── Phase 15.4: High-Fidelity Signal Recognition ───
                        // Getting ANY response (even 403/429) means the SOCKS tunnel is open.
                        // In high-concurrency environments, Tor exit nodes are often challenged.
                        port_verified = true;
                        break;
                    }
                }
                ...
                // ─── Phase 15.4: Sovereign Rescue Guard ───
                // Only trigger a service restart ONCE per mission if the connection is TRULY unreachable
                if retry_count == 0 && !port_verified && !rescue_attempted {
                    if !config.quiet {
                        println!("  ⏳ Port {} responded but SOCKS tunnel is unreachable. Tor might be stalled.", port);
                        println!("  🚀 Launching Sovereign Rescue Protocol (One-Time)...");
                    }
                    attempt_tor_service_recovery().await;
                    rescue_attempted = true;
                } else if retry_count == 0 && !port_verified && rescue_attempted && !config.quiet {
                    println!(
                        "  ⏳ Tor is still in circuit-boot phase. Exercising Sovereign Patience..."
                    );
                }

                if retry_count < max_retries {
                    retry_count += 1;
                    let wait_secs = retry_count * 15; // Increased: 15, 30, 45... total ~90s
                    if !config.quiet {
                        println!("  ⏳ Waiting for Tor Bootstrap (Attempt {}/{})... Next verification in {}s.", retry_count, max_retries, wait_secs);
                    }
                    tokio::time::sleep(Duration::from_secs(wait_secs)).await;
                } else {
                    diagnostic_log.push(format!("Port {}: SOCKS handshake failed after {} retries (Insufficient bootstrap time)", port, max_retries));
                    break;
                }
            }
        ...
        if let Some(port) = discovered_port {
            if !config.quiet {
                println!(
                    "✅ Tor found and verified on port {}. Anonymity tunnel locked.",
                    port
                );
            }
            active_client_builder = active_client_builder.proxy(Proxy::all(&final_tor_address)?);
        } else {
            // ─── Phase 9: Stealth Bridge Fallback ───
            if !config.quiet {
                println!(
                    "\n{} {} Tor Absolute Resilience Protocol failed.",
                    "⚠".bright_red().bold(),
                    "ANONYMITY ALERT //".bold()
                );
                for log in &diagnostic_log {
                    println!("  ↳ {}", log.dimmed());
                }
            }

            let mut bridge_established = false;
            if config.dynamic_proxies || config.use_proxies {
                if !config.quiet {
                    println!("🌐 [STEALTH] Attempting to establish Sovereign Proxy Bridge...");
                }

                // Early harvest to see if we can maintain anonymity without Tor
                if let Ok(proxies) = harvest_proxies_early().await {
                    if !proxies.is_empty() {
                        // Verify at least one bridge proxy
                        for p_url in proxies.iter().take(5) {
                            if let Ok(proxy_obj) = Proxy::all(p_url) {
                                if let Ok(bridge_client) = ClientBuilder::new()
                                    .proxy(proxy_obj)
                                    ...
                                    .build()
                                {
                                    if tokio::time::timeout(
                                        ...
                                        bridge_client.get("http://api.ipify.org").send(),
                                    )
                                    .await
                                    .is_ok()
                                    {
                                        if !config.quiet {
                                            println!("✅ Stealth Bridge Established via {}. Continuing via identity-obfuscation pool.", "Premium Proxy Layer".bright_green());
                                        }
                                        // Set the builder to use proxies instead of Tor
                                        active_client_builder =
                                            active_client_builder.proxy(Proxy::all(p_url)?);
            ...
            if !bridge_established {
                if config.tor_fallback {
                    println!(
                        "\n{} FALLING BACK to direct execution (Anonymity compromised).",
                        "⚠".bright_red().bold()
                    );
                } else {
                    println!(
                        "\n{} MISSION ABORTED: No functional anonymity layer available.",
                        "✗".bright_red().bold()
                    );
                    println!(
                        "💡 {} To fix your local Tor service, try:",
                        "RECOVERY HINT //".bold().bright_blue()
                    );
                    let is_termux = std::env::var("PREFIX")
                        .map(|p| p.contains("com.termux"))
                        .unwrap_or(false);
                    if is_termux {
                        println!("  1. sv restart tor (requires termux-services)");
                        if let Ok(prefix) = std::env::var("PREFIX") {
                            println!(
                                "  2. Ensure 'SocksPort 9050' is present in {}/etc/tor/torrc",
                                prefix
                            );
                        } else {
                            println!("  2. Ensure 'SocksPort 9050' is present in torrc config");
                        }
                        println!("  3. Run 'pkg install tor' or 'pkg install termux-services' if missing.");
                    } else {
                        println!("  1. sudo systemctl restart tor");
                        println!("  2. Ensure 'SocksPort 9050' is present in /etc/tor/torrc");
                        println!("  3. Run 'nyx' to check if Tor is waiting for bootstrapping.");
                    }
                    return Err(anyhow!("Tor & Stealth Bridge failed. Aborting to prevent IP leakage.\nDiagnostic: {}", diagnostic_log.join(" | ")));
                }
            }
        }
    }

    let client = active_client_builder.build()?;
    ...
    // Initialize High-Performance Architecture (2048-tier Industrial Sharding)
    let discovered = Arc::new(DashMap::with_shard_amount(2048));
    let tried_bloom = Arc::new(DashMap::with_shard_amount(2048));
    let word_bloom = Arc::new(DashMap::with_shard_amount(2048));
    let dns_cache = Arc::new(DashMap::with_shard_amount(2048));
    ...
    let state = Arc::new(AppState {
        ...
        client_pool: Arc::new(DashMap::with_shard_amount(2048)),
        default_client: client,
        discovered,
        all_results: Arc::new(DashMap::with_shard_amount(2048)),
        sources: Arc::new(DashMap::with_shard_amount(512)),
        techniques: Arc::new(DashMap::with_shard_amount(512)),
        ...
        dns_cache,
        checkpoint_path,
        wildcard_ips: Arc::new(DashMap::with_shard_amount(1024)),
        wildcard_domains: Arc::new(DashMap::with_shard_amount(1024)),
        processed_count: Arc::new(AtomicUsize::new(0)),
        error_count: Arc::new(AtomicUsize::new(0)),
        consecutive_errors: Arc::new(DashMap::with_shard_amount(1024)),
        backoff_times: Arc::new(DashMap::with_shard_amount(1024)),
        rate_limited: Arc::new(DashMap::with_shard_amount(1024)),
        robots_cache: Arc::new(DashMap::with_shard_amount(1024)),
        proxy_health: Arc::new(DashMap::with_shard_amount(1024)),
        shutdown: Arc::new(AtomicBool::new(false)),
        // Phase 1: Initialize pre-compiled regexes
        subdomain_regex: SUBDOMAIN_RE.clone(),
        html_title_regex: HTML_TITLE_RE.clone(),
        js_regex: JS_RE.clone(),
        map_regex: MAP_RE.clone(),
        csrf_regex: CSRF_RE.clone(),
        dynamic_subdomain_regex: Arc::new(DashMap::with_shard_amount(1024)),
    });
...
async fn create_resolver(
    ...
    // ─── Phase 17: Sovereign Resolver Sampling ───
    // Shuffling 62k+ resolvers ensuring every mission utilize a different 'Sovereign Subset'
    let mut sampled_resolvers = resolvers.to_vec();
    {
        let mut rng = rand::rng();
        sampled_resolvers.shuffle(&mut rng);
    }

    // Cap the active pool to 500 premium nodes to avoid library-level bookkeeping overhead
    let active_pool = if sampled_resolvers.len() > 500 {
        &sampled_resolvers[..500]
    } else {
        &sampled_resolvers[..]
    };
...
async fn harvest_proxies_early() -> Result<Vec<String>> {
    let mut proxies = HashSet::new();
    let client = Client::new();

    // ─── Phase 10: Elite GitHub-Hosted Proxy Sources ───
    let bridge_sources = [
            ...
            if let Ok(text) = resp.text().await {
                for line in text.lines() {
                    ...
                    if p.len() > 5 && p.contains(':') {
                        // All these lists are raw IP:Port, mostly SOCKS5
                        proxies.insert(format!("socks5://{}", p));
                    }
                }
            }
        }
...
async fn is_wildcard_elite(state: &AppState, subdomain: &str, ips: &[String]) -> bool {
    // 1. Check if ANY IP is in the known-bad wildcard set (DashMap check is O(1))
    let mut is_wildcard = ips.iter().any(|ip| state.wildcard_ips.contains_key(ip));

    // 2. Phase 3: Elite Wildcard Response Fingerprinting
    // If the IPs match a wildcard but we're in 'Absolute Depth' mode, verify with a web handshake
    if is_wildcard && !state.config.quiet && !state.config.stealth_mode {
        let url = format!("http://{}", subdomain);
        if let Ok(resp) = smart_fetch(state, &url, "Wildcard-Verification".to_string()).await {
            if resp.status().is_success() {
                let len = resp.content_length().unwrap_or(0);
                // Standard wildcard mirrors are usually constant and small (<5kb).
                // Real assets like 'dev.google.com' often have unique content sizes.
                is_wildcard = len < 5000;
            }
        }
    }
    is_wildcard
...
async fn attempt_tor_service_recovery() {
    // Phase 15: Sovereign Service Stabilization
    // Detect environment and available service manager
    let is_termux = std::env::var("PREFIX")
        .map(|p| p.contains("com.termux"))
        .unwrap_or(false);

    if is_termux {
        // Termux: Try `sv` (termux-services) or standard backgrounding
        if std::process::Command::new("which")
            ...
            .unwrap_or(false)
        {
            let _ = tokio::process::Command::new("sv")
                .arg("restart")
                .arg("tor")
                .status()
                .await;
        } else {
            // Fallback: kill and restart tor manually if service manager is missing
            let _ = tokio::process::Command::new("pkill")
                .arg("-f")
                .arg("tor")
                .status()
                .await;
            let _ = tokio::process::Command::new("tor").arg("--quiet").spawn();
        }
        ...
        .unwrap_or(false)
    {
        ...
        .unwrap_or(false)
    {
        // Standard Linux (SysVinit/OpenRC)
        let _ = tokio::process::Command::new("sudo")
            .arg("-n")
            .arg("service")
            .arg("tor")
    ...
    // Wait for the service to bind and circuit to initialize
    // Tor often needs a moment to actually bind to the port before SOCKS is ready.
...
async fn rotate_user_agent(state: &AppState) -> Option<String> {
    ...
    // ELITE STEALTH: Randomize rotation
    let mut rng = rand::rng();
...
async fn load_proxies(state: Arc<AppState>) -> Result<()> {
    ...
    // ─── Phase 16: Sovereign Identity Guard ───
    // Fetch local IP to detect identity leaks during proxy validation
    if !state.config.quiet {
        println!("📡 [IDENTITY] Establishing Sovereign Guard baseline...");
    }
    let local_ip = match fetch_local_ip().await {
        Ok(ip) => {
            if !state.config.quiet {
                println!(
                    "✅ [IDENTITY] Identity Guard locked. Baseline: {}.",
                    ip.bright_green()
                );
            }
            Some(ip)
        ...
        Err(_e) => {
            if !state.config.quiet {
                println!(
                    "⚠️  Identity Guard Warn: Baseline capture failed. Leak checking limited."
                );
            }
            None
        }
    ...
    if state.config.proxy_test && !proxies.is_empty() {
        ...
        let tested_proxies = test_proxies(&state, proxies, local_ip).await;
        let mut proxy_lock = state.proxies.write().await;
        *proxy_lock = tested_proxies;

        if !state.config.quiet && !proxy_lock.is_empty() {
            println!(
                "✅ [SOVEREIGN] Identified {} premium nodes. Priority: FASTEST-FIRST.",
                proxy_lock.len()
            );
        }
    } else {
...
/// 🛰️ SOVEREIGN IDENTITY GUARD: Fetch local public IP for leak detection
async fn fetch_local_ip() -> Result<String> {
    ...
    for target in PROXY_VERIFY_TARGETS {
        let client_clone = client.clone();
        futures.push(async move {
            if let Ok(resp) = client_clone.get(*target).send().await {
                if let Ok(text) = resp.text().await {
    ...
    Err(anyhow!("Failed to establish Identity Guard baseline"))
}
...
async fn test_proxies(
    ...
    let mut working_with_latency: Vec<(String, Duration)> = Vec::new();
    ...
    let mut stream = stream::iter(proxies_to_test)
        .map(|proxy_url| {
            ...
            async move {
                ...
                // Rotate validation targets to prevent throttling
                let target_url =
                    PROXY_VERIFY_TARGETS[rand::rng().random_range(0..PROXY_VERIFY_TARGETS.len())];
                ...
                match client.get(target_url).send().await {
                    Ok(response) if response.status().is_success() => {
                        ...
                        // Identity Guard: Leak Check
                        if let (Some(lip), Ok(body)) = (local_ip_clone, response.text().await) {
                            if body.contains(&lip) {
                                // Transparent Proxy: Identity Leaked!
                                return None;
                            }
                        }

                        Some((proxy_url, latency))
                    }
                    _ => None,
                }
            }
        })
        .buffer_unordered(250); // Velocity Tier: 250 parallel tests
    ...
    while let Some(res) = stream.next().await {
        ...
        if let Some(entry) = res {
            working_with_latency.push(entry);
        }

        if !state.config.quiet && tested_count % 50 == 0 {
            println!(
    ...
    if !state.config.quiet {
        println!("📡 [PROXIES] Validation Complete. Measuring Final Latency Distribution...");
    }
...
fn log_mission_header(domain: &str) {
    ...
    println!(
        " {} {} {}",
        "❱".cyan().bold(),
        "MISSION ADVISORY — TARGET:".bright_white().bold(),
        domain.bright_yellow().bold()
    );
    println!(
        " {} {} {}",
        "❱".cyan().bold(),
        "SOVEREIGN ENGINE :".dimmed(),
        "v0.1.0-QUANTUM // APEX".bright_white()
    );
    println!(
        " {} {} {}",
        "❱".cyan().bold(),
...
async fn enumerate_passive_sources(
    ...
    while let Some(res) = results_stream.next().await {
        // Abort check: stop consuming passive sources if mission is cancelled
        if crate::signals::is_aborted() {
            break;
        }
...
/// Zero-allocation domain regex cache. Compiles once per domain, reused everywhere.
fn get_or_create_domain_regex(state: &AppState, domain: &str) -> Result<Regex> {
    if let Some(cached) = state.dynamic_subdomain_regex.get(domain) {
        return Ok(cached.value().clone());
    }
    let pattern = format!(r"([a-zA-Z0-9][a-zA-Z0-9.-]+\.{})", regex::escape(domain));
    let re = Regex::new(&pattern)?;
    state
        .dynamic_subdomain_regex
        .insert(domain.to_string(), re.clone());
    Ok(re)
}
...
async fn parse_source_response(
    ...
    // Pre-cache domain regex (zero-allocation on cache hit)
    let domain_re = get_or_create_domain_regex(state, domain)?;
    ...
    match source.parser {
                ...
                if let Some(passive_dns) = data.get("passive_dns").and_then(|v| v.as_array()) {
                    for entry in passive_dns {
                        if let Some(hostname) = entry.get("hostname").and_then(|v| v.as_str()) {
                            let sub = hostname.to_ascii_lowercase();
                            if sub.ends_with(domain) {
                                subdomains.push(sub);
                            }
                        }
        ...
        ParserType::DnsDumpster => {
            // Use cached domain regex instead of runtime compilation
            for cap in domain_re.captures_iter(&text) {
                subdomains.push(cap[1].to_ascii_lowercase());
            }

            // Attempt to extract CSRF for deeper scraping if needed
            if let Some(_csrf) = extract_csrf_token(state, &text) {
                ...
                if let Some(subs) = data.get("subdomains").and_then(|v| v.as_array()) {
                    for sub in subs {
                        if let Some(sub_str) = sub.as_str() {
                            subdomains.push(sub_str.to_ascii_lowercase());
                        }
                    }
                }
            }
                    ...
                    for result in results {
                        if let Some(page) = result.get("page") {
                            if let Some(domain_val) = page.get("domain").and_then(|v| v.as_str()) {
                                subdomains.push(domain_val.to_ascii_lowercase());
                            }
                        }
                    }
                }
                ...
                if let Some(records) = data.get("FDNS_A").and_then(|v| v.as_array()) {
                    for record in records {
                        if let Some(record_str) = record.as_str() {
                            // BufferOver format is "ip,hostname" — extract hostname (index 1)
                            if let Some(sub) = record_str.split(',').nth(1) {
                                let sub = sub.trim().to_ascii_lowercase();
                                if sub.ends_with(domain) {
                                    subdomains.push(sub);
                                }
            ...
            if let Ok(entries) = serde_json::from_str::<Vec<Vec<String>>>(&text) {
                for entry in entries.iter().skip(1) {
                    if let Some(url_str) = entry.first() {
                        if let Ok(parsed) = Url::parse(url_str) {
                            if let Some(host) = parsed.host_str() {
                                subdomains.push(host.to_ascii_lowercase());
                            }
                        }
                    }
                }
                    ...
                    for item in items {
                                ...
                                if full_name.contains(domain) {
                                    ...
                                    if let Some(name) = parts.last() {
                                        if name.ends_with(domain) {
                                            subdomains.push(name.to_ascii_lowercase());
                                        }
                                    }
                                }
                            }
                        ...
                        if let Some(path) = item.get("path").and_then(|v| v.as_str()) {
                            if path.contains(domain) {
                                for cap in domain_re.captures_iter(path) {
                                    subdomains.push(cap[1].to_ascii_lowercase());
                                }
                            }
                        }
                ...
                if let Some(data_arr) = data.get("data").and_then(|v| v.as_array()) {
                    for item in data_arr {
                        if let Some(sub) = item.get("id").and_then(|v| v.as_str()) {
                            subdomains.push(sub.to_ascii_lowercase());
                        }
                    }
                }
            }
        ...
        ParserType::HtmlTable => {
            // Zero-allocation: Use cached domain regex instead of recompiling per-call
            for cap in domain_re.captures_iter(&text) {
                subdomains.push(cap[1].to_ascii_lowercase());
            }
        }
        ...
        ParserType::RecursiveGrep => {
            ...
            for cap in domain_re.captures_iter(&unescaped) {
                subdomains.push(cap[1].to_ascii_lowercase());
            }
        }

        ParserType::Custom | ParserType::Robtex | ParserType::SecurityTrails => {
            // Default to cached domain regex for safety
            for cap in domain_re.captures_iter(&text) {
...
async fn enumerate_dns_bruteforce(state: Arc<AppState>, domain: String) -> Result<()> {
    ...
    while let Some(word) = word_rx.recv().await {
        ...
        let handle = tokio::spawn(async move {
            ...
            if state.tried_bloom.contains_key(&hash_string(&subdomain)) {
                return Ok::<(), anyhow::Error>(());
            }

            // Pre-filtering check: Skip if already in results
            if state.discovered.contains_key(&subdomain) {
                return Ok::<(), anyhow::Error>(());
            }

            // Mark as tried globally
            state.tried_bloom.insert(hash_string(&subdomain), ());
            ...
            if !state.config.quiet && q > 0 && q % 10000 == 0 {
                println!(
        ...
        // Drain completed handles periodically to avoid unbounded Vec growth
        if task_handles.len() >= 2000 {
            let mut remaining = Vec::with_capacity(task_handles.len());
    ...
    // Wait for all remaining spawned tasks to complete
...
async fn produce_wordlist_stream(
    ...
    for word in priorities {
        ...
        if !state.word_bloom.contains_key(&hash_string(&word)) {
            state.word_bloom.insert(hash_string(&word), ());
    ...
    for &word_ref in BUILTIN_WORDLIST {
        ...
        let is_duplicate = state.word_bloom.insert(hash_string(&word), ()).is_some();
    ...
    tokio::task::spawn_blocking(move || -> Result<()> {
        ...
        for line in reader.lines().map_while(Result::ok) {
            ...
            if !word.is_empty() && !word.starts_with('#') {
                // QUANTUM DEDUPLICATION: Check word DashSet
                let is_duplicate = state_clone
                    .word_bloom
                    .insert(hash_string(&word), ())
                    .is_some();

                if !is_duplicate && tx.blocking_send(word).is_err() {
                    break;
                }
            }
        }
...
async fn load_wordlist_from_url_stream(
    ...
    while let Some(chunk) = resp.chunk().await? {
        ...
        // Log progress every 5MB or 5 seconds for large files (respects --quiet)
        if !state.config.quiet
            && (last_log.elapsed().as_secs() >= 5 || chunk.len() > 5 * 1024 * 1024)
        {
    ...
    let word = buffer.trim().to_ascii_lowercase();
    if !word.is_empty() && !word.starts_with('#') {
        // QUANTUM DEDUPLICATION: Check word DashSet
        if state.word_bloom.insert(hash_string(&word), ()).is_none() {
...
async fn enumerate_permutations(
    ...
    stream::iter(permutations)
        .map(|p_sub| {
            let state = state.clone();
            async move {
                let _ = add_result_dns_checked(&state, p_sub, "Elite-Mutations", "ALTDNS").await;
                Ok::<(), anyhow::Error>(())
            }
        })
        .buffer_unordered(state.config.concurrency)
...
async fn enumerate_cloud_recon(state: Arc<AppState>, root_domain: String) -> Result<()> {
    ...
    let mut common_buckets = HashSet::new();

    // Dynamic Cloud Permutations (Phase 15 - Elite)
    let seeds = [
        "",
        "-assets",
        "-backup",
    ...
    for seed in seeds {
        if seed.is_empty() {
            common_buckets.insert(base.to_string());
        } else if seed.starts_with('-') {
            common_buckets.insert(format!("{}{}", base, seed));
            common_buckets.insert(format!("{}{}", root_domain.replace('.', "-"), seed));
        }
    }

    // Reverse patterns
    common_buckets.insert(format!("assets-{}", base));
    common_buckets.insert(format!("static-{}", base));
    common_buckets.insert(format!("media-{}", base));
            ...
            for url in urls {
                if let Ok(resp) = smart_fetch(&state, &url, "Cloud-Recon".to_string()).await {
                    if resp.status().is_success() || resp.status() == StatusCode::FORBIDDEN {
                        ...
                        let target = bucket.to_string() + "." + &domain_root;
                        // Cloud DNS Validation: Ensure it resolves to actual infrastructure
                        if let Ok(Some((ips, _))) = resolve_domain_smart(&state, &target).await {
                            if !ips.is_empty() {
...
async fn enumerate_hidden_assets(state: Arc<AppState>, domain: String) -> Result<()> {
    ...
    let live_subs: Vec<String> = {
        state
            .all_results
            .iter()
            .filter(|entry| entry.value().http_status.is_some_and(|s| s < 400))
        ...
        .map(|sub| {
            ...
            async move {
                ...
                for word in common {
                    ...
                    let _ = add_result_dns_checked(&state, deeper, &source, "RECURSIVE").await;
                }
                Ok::<(), anyhow::Error>(())
            }
        })
    ...
    for base_url in urls_to_try {
        ...
        if let Ok(response) = smart_fetch(state, &base_url, source.to_string()).await {
            if let Ok(text) = response.text().await {
                ...
                for cap in state.js_regex.captures_iter(&text) {
                ...
                if text.contains(".map") {
                    for cap in state.map_regex.captures_iter(&text) {
...
async fn extract_subdomains_from_text(
    ...
    // Match subdomains in text using cached domain-specific regex
    let re = if let Some(cached) = state.dynamic_subdomain_regex.get(root_domain) {
        cached.value().clone()
    } else {
        let pattern = format!(
        ...
        state
            .dynamic_subdomain_regex
            .insert(root_domain.to_string(), new_re.clone());
        new_re
    };

    for cap in re.captures_iter(text) {
        let found = cap[1].to_ascii_lowercase();
...
async fn enumerate_vhost_discovery(state: Arc<AppState>, domain: String) -> Result<()> {
    ...
    // ─── Phase 8: Sovereign Discovery Pivot (VHost) ───
    // Elite: Instead of a static list, we use EVERY subdomain discovered so far as a Host candidate
    let mut candidate_vhosts: Vec<String> = state
        .discovered
        .iter()
        .map(|entry| entry.key().clone())
        .take(200) // Safety throttle for ultra-large targets
        .collect();

    // Add common defaults as fallback
    let defaults = [
        "www", "api", "admin", "dev", "test", "stage", "prod", "internal",
    ];
    for d in defaults {
        let full = format!("{}.{}", d, domain);
        if !candidate_vhosts.contains(&full) {
            candidate_vhosts.push(full);
        }
    }

    if !state.config.quiet {
        println!(
            "🏠 Executing Sovereign VHost Pivot ({} IPs vs {} candidate hostnames)...",
            ips.len(),
            candidate_vhosts.len()
        );
    }
    ...
    stream::iter(ips)
        .map(|ip| {
            ...
            let candidate_vhosts = candidate_vhosts.clone();
            async move {
                for test_sub in candidate_vhosts {
                    if crate::signals::is_aborted() {
                        break;
                    }
                    for proto in ["http", "https"] {
                        ...
                        if let Ok(val) = reqwest::header::HeaderValue::from_str(&test_sub) {
                            headers.insert(reqwest::header::HOST, val);
                        }

                        if let Ok(resp) = smart_fetch_with_headers(
                            &state,
                            &url,
                            "VHost-Discovery".to_string(),
                            Some(headers),
                        )
                        .await
                        {
                            if resp.status().is_success() {
                                // Basic exclusion: if the IP responds the same way without the Host header, it's not a VHost
                                if let Ok(default_resp) =
                                    smart_fetch(&state, &url, "VHost-Discovery".to_string()).await
                                {
                                    if default_resp.status() != resp.status()
                                        || default_resp.content_length() != resp.content_length()
                                    {
                                        let _ = add_result_with_resolution(
                                            &state,
                                            test_sub.clone(),
...
async fn enumerate_axfr_internal(state: Arc<AppState>, domain: String) -> Result<()> {
    ...
    // ZERO DNS LEAKAGE: AXFR requires raw TCP to port 53 — cannot be routed through proxies/Tor
    if state.config.use_tor || state.config.use_proxies || state.config.stealth_mode {
        if state.config.verbose {
            println!(
                "⚠️  [SKIP] AXFR-Transfer skipped: raw DNS queries would leak IP through proxy/Tor"
            );
        }
        return Ok(());
    }
...
async fn enumerate_ent_discovery(state: Arc<AppState>, domain: String) -> Result<()> {
    ...
    // ZERO DNS LEAKAGE: ENT discovery uses raw DNS ANY queries — bypass when anonymity active
    if state.config.use_tor || state.config.use_proxies || state.config.stealth_mode {
        if state.config.verbose {
            println!(
                "⚠️  [SKIP] ENT-Discovery skipped: raw DNS queries would leak IP through proxy/Tor"
            );
        }
        return Ok(());
    }
        ...
        .map(|sub| {
            ...
            async move {
                ...
                if parts.len() > 2 {
                    ...
                    if !state.discovered.contains_key(&parent) {
...
async fn enumerate_nsec_walking(state: Arc<AppState>, domain: String) -> Result<()> {
    ...
    // ZERO DNS LEAKAGE: NSEC walking uses raw DNS protocol — bypass when anonymity active
    if state.config.use_tor || state.config.use_proxies || state.config.stealth_mode {
        if state.config.verbose {
            println!(
                "⚠️  [SKIP] NSEC-Walking skipped: raw DNS queries would leak IP through proxy/Tor"
            );
        }
        return Ok(());
    }
...
async fn enumerate_nsec3_walking(state: Arc<AppState>, domain: String) -> Result<()> {
    ...
    // ZERO DNS LEAKAGE: NSEC3 walking uses raw DNS protocol — bypass when anonymity active
    if state.config.use_tor || state.config.use_proxies || state.config.stealth_mode {
        if state.config.verbose {
            println!(
                "⚠️  [SKIP] NSEC3-Walking skipped: raw DNS queries would leak IP through proxy/Tor"
            );
        }
        return Ok(());
    }

    // ─── Phase 10: Iterative NSEC3 Zone Mapping ───
    let mut current_hash_prefix = "0".to_string(); // Start with any low hash
    ...
    while iterations < 100 {
        ...
        let probe = format!("{}.{}", current_hash_prefix, domain);
        if let Ok(lookup) = state
            .dns_resolver
            .lookup(probe.as_str(), RecordType::NSEC3)
            .await
        {
            let mut found_new = false;
            for record in lookup.iter() {
                let rec_str = record.to_string().to_ascii_lowercase();
                let found = extract_subs_from_text(&rec_str, &domain);
                for f in found {
                    if state.discovered.insert(f.clone(), ()).is_none() {
                        let _ = add_result_with_resolution(&state, f, source, "NSEC3").await;
                        found_new = true;
                    }
                }

                // Extract "Next" hash if available in record string for iterative walk
                if let Some(next_hash) = rec_str.split_whitespace().last() {
                    if next_hash.len() > 10 && next_hash != current_hash_prefix {
                        current_hash_prefix = next_hash.to_string();
                        found_new = true;
                    }
                }
            }
...
async fn enumerate_tls_san(state: Arc<AppState>, domain: String) -> Result<()> {
    ...
    stream::iter(targets)
        .map(|(ip, port)| {
            ...
            let domain = domain.clone();
            async move {
                // Use smart_fetch through proxy stack instead of raw default_client
                let url = format!("https://{}:{}", ip, port);
                match smart_fetch(&state, &url, "TLS-SAN".to_string()).await {
                    Ok(_) => {}
                    Err(e) => {
...
async fn enumerate_ptr_expansion(state: Arc<AppState>, domain: String) -> Result<()> {
    ...
    // ZERO DNS LEAKAGE: PTR expansion uses raw reverse DNS — bypass when anonymity active
    if state.config.use_tor || state.config.use_proxies || state.config.stealth_mode {
        if state.config.verbose {
            println!(
                "⚠️  [SKIP] PTR-Expansion skipped: raw DNS queries would leak IP through proxy/Tor"
            );
        }
        return Ok(());
    }

    // ─── Phase 10: Organization CIDR / RDAP Sweep ───
    if !state.config.quiet {
        println!("🌐 Starting Tactical Organization CIDR Sweep (RDAP-Based Architecture)...");
    }

    let mut blocks = HashSet::new();

    // 1. Pivot from discovered IPs to RDAP CIDR blocks
    let discovered_ips: Vec<String> = state
        .all_results
        .iter()
        .flat_map(|entry| entry.value().resolved_ips.clone())
        .collect();

    for ip in discovered_ips.iter().take(20) {
        // Throttle RDAP lookups
        if crate::signals::is_aborted() {
            break;
        }
        let rdap_url = format!("https://rdap.db.ripe.net/ip/{}", ip); // Simplified pivot for RIPE/Global
        if let Ok(resp) = state.default_client.get(&rdap_url).send().await {
            if let Ok(json) = resp.json::<Value>().await {
                if let Some(cidr_str) = json.get("handle").and_then(|h| h.as_str()) {
                    // Very often the handle or title contains CIDR or we can parse from 'ipNet'
                    if cidr_str.contains('/') {
                        blocks.insert(cidr_str.to_string());
                    }
                }
            }
        }
    ...
    // Fallback to /24 discovery if RDAP yielded nothing or for diversity
    ...
    stream::iter(blocks)
        .map(|block| {
            ...
            let domain = domain.clone();
            async move {
                // Parse start/end or just iterate /24 for speed
                let prefix = if block.contains('/') {
                    block
                        .split('/')
                        .next()
                        .unwrap_or("")
    ...
    stream::iter(wordlist)
        .map(|word| {
            ...
            async move {
                ...
                let _ = add_result_dns_checked(&state, target, source, "QUANTUM-BRUTE").await;
                Ok::<(), anyhow::Error>(())
            }
        })
        .buffer_unordered(state.config.concurrency.min(500))
        .collect::<Vec<_>>()
        .await;
...
async fn enumerate_dns_axfr(state: Arc<AppState>, domain: String) -> Result<()> {
    ...
    // ZERO DNS LEAKAGE: AXFR uses raw DNS protocol — bypass when anonymity active
    if state.config.use_tor || state.config.use_proxies || state.config.stealth_mode {
        if state.config.verbose {
            println!(
                "⚠️  [SKIP] DNS-AXFR skipped: raw DNS queries would leak IP through proxy/Tor"
            );
        }
        return Ok(());
    }
...
async fn enumerate_dns_ptr(state: Arc<AppState>, domain: String) -> Result<()> {
    ...
    // ZERO DNS LEAKAGE: PTR uses raw reverse DNS — bypass when anonymity active
    if state.config.use_tor || state.config.use_proxies || state.config.stealth_mode {
        if state.config.verbose {
            println!("⚠️  [SKIP] DNS-PTR skipped: raw DNS queries would leak IP through proxy/Tor");
        }
        return Ok(());
    }
...
async fn smart_fetch(state: &AppState, url: &str, source: String) -> Result<Response> {
    smart_fetch_with_headers(state, url, source, None).await
}

async fn smart_fetch_with_headers(
    state: &AppState,
    url: &str,
    source: String,
    ...
    if let Some(backoff_until) = state.backoff_times.get(&source) {
        if Instant::now() < *backoff_until {
            let wait = backoff_until.saturating_duration_since(Instant::now());
    ...
    let client = if let Some(p_url) = &proxy_url {
        // ─── Phase 13: Sovereign Circuit Breaker ───
        if let Some(mut health) = state.proxy_health.get_mut(p_url) {
            if health.is_burned || (health.failure_count > 15 && health.success_count < 2) {
                if !health.is_burned {
                    health.is_burned = true;
                    if state.config.verbose {
                        println!(
                            "💀 [CIRCUIT BREAKER] Proxy {} marked as BURNED. Identity scrubbed.",
                            p_url
                        );
                    }
                }
                state.current_proxy_index.fetch_add(1, Ordering::Relaxed);
                return Err(anyhow!("Proxy Identity Burned"));
            }
        }
        ...
        if let Some(c) = state.client_pool.get(p_url) {
            ...
            let new_client = ClientBuilder::new()
                ...
                .pool_max_idle_per_host(32) // Ultra-high industrial grade
                .danger_accept_invalid_certs(true)
    ...
    for attempt in 0..=state.config.retries {
        // Apply jitter ONLY when stealth mode is active or source is rate-limited
        // This eliminates hours of dead wait time during high-throughput brute-force phases
        let jitter = if state.config.stealth_mode || state.rate_limited.contains_key(&source) {
            let j = rand::random_range(
        ...
        let mut request = if source == "Web-Analysis" {
            ...
            if s.method == Method::POST {
                // Industry-Grade POST: Send domain as form body for sources that require it
                client
                    .post(url)
        ...
        match request.send().await {
            Ok(response) => {
                ...
                // ─── Phase 13: Proxy Success Tracking ───
                ...
                if status == StatusCode::TOO_MANY_REQUESTS || status == StatusCode::FORBIDDEN {
                    if !state.config.quiet {
                        println!("🔄 [IDENTITY] Signature blocked by {} (Status {}). Rotating identity...", source, status);
                    }

                    let backoff = Duration::from_secs(rand::random_range(60..300));
                    state
                        .backoff_times
                        .insert(source.clone(), Instant::now() + backoff);
                    state.rate_limited.insert(source.clone(), true);

                    if let Some(p_url) = &proxy_url {
                        let mut health = state.proxy_health.entry(p_url.clone()).or_default();
                        health.failure_count += 5; // Heavily penalize identity blocks
                    }

                    state.current_proxy_index.fetch_add(1, Ordering::Relaxed);
                    last_error = Some(anyhow!("Rate Limited (Status {})", status));
            ...
            Err(e) => {
                // ─── Phase 13: Proxy Failure Tracking ───
                if let Some(p_url) = &proxy_url {
                    let mut health = state.proxy_health.entry(p_url.clone()).or_default();
                ...
                *err_count += 1;
                ...
                if *err_count > 5 {
                    let backoff = Duration::from_secs(rand::random_range(30..120));
                    state
                        .backoff_times
                        .insert(source.clone(), Instant::now() + backoff);
                }
...
async fn resolve_domain_doh(
    ...
    // Phase 3: Zero DNS Leakage via DoH over Proxy/Tor. Concurrently fetch A and AAAA records.
    let url_a = format!("https://dns.google/resolve?name={}&type=A", domain);
    let url_aaaa = format!("https://dns.google/resolve?name={}&type=AAAA", domain);
    let (a_res, aaaa_res) = tokio::join!(
        smart_fetch(state, &url_a, "DoH".to_string()),
        smart_fetch(state, &url_aaaa, "DoH".to_string())
    );

    for response in [a_res, aaaa_res].into_iter().flatten() {
        if let Ok(text) = response.text().await {
            if let Ok(json) = serde_json::from_str::<Value>(&text) {
                if let Some(answers) = json.get("Answer").and_then(|a| a.as_array()) {
                    for ans in answers {
                        if let Some(t) = ans.get("type").and_then(|t| t.as_u64()) {
                            if t == 1 || t == 28 {
                                if let Some(ip) = ans.get("data").and_then(|d| d.as_str()) {
                                    ips.push(ip.to_string());
                                }
                            } else if t == 5 {
                                if let Some(cname) = ans.get("data").and_then(|d| d.as_str()) {
                                    if cname_str.is_none() {
                                        cname_str = Some(cname.trim_end_matches('.').to_string());
                                    }
                                }
                            }
                        }
...
async fn resolve_domain_smart(
    ...
    // L1 Caching: Zero-Latency redundant lookups
    if let Some(cached) = state.dns_cache.get(domain) {
        return Ok(cached.clone());
    }

    // ZERO DNS LEAKAGE: Route through DoH when anonymity is required
    let mut result =
        if state.config.use_tor || state.config.use_proxies || state.config.stealth_mode {
            resolve_domain_doh(state, domain).await
        } else {
            resolve_domain_smart_internal(state, domain).await
        };

    // ─── Phase 13: High-Precision DNS Failover (Tier 2 Sovereign Fallback) ───
    if result.as_ref().map(|r| r.is_none()).unwrap_or(true) && !state.config.stealth_mode {
        if state.config.verbose {
            println!(
                "🛰️  [FAILOVER] Tier 1 Resolution failed for {}. Triggering Sovereign DoH...",
                domain
            );
        }
        result = resolve_domain_doh(state, domain).await;
    }

    if let Ok(ref res) = result {
        state.dns_cache.insert(domain.to_string(), res.clone());
    }
    result
}

async fn resolve_domain_smart_internal(
    state: &AppState,
    domain: &str,
) -> Result<Option<(Vec<String>, Option<String>)>> {
...
async fn add_result_dns_only(
    ...
    // Phase 4: Extreme Wildcard Validation Array
    if is_wildcard_elite(state, &subdomain, &ips).await {
        return Err(anyhow::anyhow!("Wildcard detected"));
    }
    // Deduplicate
    if state.discovered.contains_key(&subdomain) {
...
async fn add_result_dns_checked(
    state: &AppState,
    subdomain: String,
    source: &str,
    technique: &str,
...
async fn check_http_endpoint(
    ...
    let root_domain = if !state.config.domains.is_empty() {
        ...
        if parts.len() >= 2 {
            fallback_root = format!("{}.{}", parts[parts.len() - 2], parts[parts.len() - 1]);
            &fallback_root
        } else {
            subdomain
        }
    ...
    // ZERO IP LEAKAGE: Skip direct TLS handshake when anonymity is required
    if !state.config.quiet
        && subdomain.contains('.')
        && !state.config.use_tor
        && !state.config.use_proxies
        && !state.config.stealth_mode
    {
        let discovered_sans = extract_sans_binary(subdomain, root_domain).await;
        for san in discovered_sans {
            if !state.discovered.contains_key(&san) {
    ...
    for url in urls {
        if let Ok(response) = smart_fetch(state, &url, "Web-Analysis".to_string()).await {
            ...
            let (title, body_subs, response_text) = if let Ok(text) = response.text().await {
            ...
            let mut local_cache = HashSet::new();
            for group in [body_subs, header_subs] {
                for sub in group {
                    if sub.ends_with(root_domain) && local_cache.insert(sub.clone()) {
                        // Recursion Guard: Use DNS-only path for discovered subdomains to avoid probing loops
                        if !state.discovered.contains_key(&sub) {
                            if let Ok(Some((ips, cname))) = resolve_domain_smart(state, &sub).await
                            {
...
async fn extract_sans_binary(subdomain: &str, root_domain: &str) -> Vec<String> {
    ...
    // Attempt direct TCP connection with timeout to prevent indefinite hangs
    let tcp_result = tokio::time::timeout(
        Duration::from_secs(5),
        tokio::net::TcpStream::connect(format!("{}:443", subdomain)),
    )
    .await;
    if let Ok(Ok(stream)) = tcp_result {
...
async fn detect_wildcard_dns(state: &AppState, domain: &str) -> Result<()> {
    ...
    if resolved_count >= WILDCARD_TEST_THRESHOLD {
...
fn extract_csrf_token(state: &AppState, html: &str) -> Option<String> {
    state
        .csrf_regex
        .captures(html)
...
async fn generate_mission_summary(state: &AppState) -> Result<()> {
    ...
    for entry in state.all_results.iter() {
        let sub = entry.key().to_ascii_lowercase();
...
async fn print_mission_briefing_summary(state: &AppState) {
    ...
    println!("║ {} ║", "SOVEREIGN INTELLIGENCE SUMMARY".bold().cyan());
    println!(
        "{}",
        "╠═════════════════════════════════════════════════════════════╣".bright_black()
    );
...
mod tests {
    ...
    fn test_extract_csrf_token() {
        ...
        // Test regex directly since extract_csrf_token requires AppState
        assert_eq!(
            CSRF_RE
                .captures(html)
                .and_then(|cap| cap.get(1))
    ...
    fn test_subdomain_pattern() {
        ...
        let re =
            Regex::new(r"([a-zA-Z0-9][a-zA-Z0-9.-]+\.example\.com)").expect("Test Pattern Failure");
