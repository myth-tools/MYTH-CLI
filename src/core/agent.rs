...
pub struct ReconAgent {
    ...
    cached_mcp_info: Arc<Mutex<Option<String>>>,
    last_mcp_sync: Arc<Mutex<Option<std::time::Instant>>>,
}
...
/// Events sent from tool bridges to the agent for immediate persistence.
#[derive(Debug, Clone)]
...
impl ReconAgent {
    ...
    pub async fn new(config: AppConfig) -> Result<Self, AgentError> {
        ...
        let nim_client =
            Arc::new(NimClient::from_config(&config).map_err(|e| AgentError::Llm(e.to_string()))?);
        ...
        Ok(Self {
            ...
            cached_mcp_info: Arc::new(Mutex::new(None)),
            last_mcp_sync: Arc::new(Mutex::new(None)),
        })
    }

    /// Construct a ReconAgent injected with context from the MissionPipeline
    pub async fn from_pipeline_context(
        config: AppConfig,
        context: Arc<crate::core::pipeline::PipelineContext>,
        mcp_server: McpServer,
        nim_client: Arc<NimClient>,
        ...
        Ok(Self {
            nim_client,
            mcp_server,
            recon_graph: context.recon_graph.clone(),
            config,
            messages: Arc::new(std::sync::Mutex::new(messages)),
            session: None,
            ...
            event_tx: None,
            missing_tools: Arc::new(tokio::sync::Mutex::new(HashSet::new())),
            cached_mcp_info: Arc::new(Mutex::new(None)),
            last_mcp_sync: Arc::new(Mutex::new(None)),
        })
    }
    ...
    pub async fn clear_session(&mut self) {
        ...
        let mcp_info = self.build_mcp_server_info().await;

        {
            let mut m = self.messages.lock().unwrap();
            if let Some(msg) = m.get_mut(0) {
                msg.content = prompts::system_prompt(
                    &self.config.agent.name,
                    &self.config.agent.version,
                    &self.config.agent.all_report_path,
                    &self.config.creator,
        ...
        self.recon_graph.lock().await.clear();
        self.memory.clear();
        self.missing_tools.lock().await.clear();
    ...
    async fn build_mcp_server_info(&self) -> String {
        {
            let last_sync = self.last_mcp_sync.lock().await;
            let cached = self.cached_mcp_info.lock().await;
            if let (Some(ts), Some(info)) = (*last_sync, cached.as_ref()) {
        ...
        let all_tools: HashMap<String, Vec<serde_json::Value>> =
        ...
        let mut cached_info = self.cached_mcp_info.lock().await;
        let mut last_sync = self.last_mcp_sync.lock().await;
    ...
    async fn build_rig_agent(
        ...
        mcp_info: &str,
        history_tx: Option<mpsc::UnboundedSender<HistoryEvent>>,
    ...
    fn build_history(&self) -> Vec<RigMessage> {
        ...
        let max_history = self.config.agent.max_history_turns;
        let skip_count = msg_count.saturating_sub(max_history);
        let messages_snapshot = {
            let m = self.messages.lock().unwrap();
            m.clone()
        };

        for (i, msg) in messages_snapshot.iter().enumerate().skip(skip_count) {
            if i == msg_count - 1 {
                continue;
            }
            match msg.role {
    ...
    pub async fn chat(&mut self, prompt: &str) -> Result<String, AgentError> {
        ...
        let (mcp_info, history) =
            tokio::join!(self.build_mcp_server_info(), async { self.build_history() });

        let ai_agent = self
            .build_rig_agent(executed_tools.clone(), workspace_path, &mcp_info, None)
            .await;
        ...
        let response =
                ...
                .await
            {
                ...
                _ => {
                    ...
                    let retry_agent = self
                        .build_rig_agent(executed_tools2, workspace_path2, &mcp_info, None)
                        .await;
    ...
    > {
        ...
        while attempts < max_attempts {
            ...
            let mut s = ai_agent.stream_chat(prompt, history.clone()).await;

            // Poll the first item to check for immediate connection errors (like 429)
            // We use a shorter timeout (30s) for the first token to detect stalls quickly.
            match tokio::time::timeout(tokio::time::Duration::from_secs(30), s.next()).await {
                Ok(Some(Err(e))) => {
                    let err_str = e.to_string();
                    if err_str.contains("429") || err_str.contains("rate limit") {
                        tracing::warn!(attempt = attempts, "Tactical: Rate limit (429) detected on first token. Rotating key and retrying.");
                        self.nim_client.rotate_key();
                        tokio::time::sleep(std::time::Duration::from_millis(500 * attempts as u64))
                            .await;
                        continue;
                    }
                    // For other errors, we re-wrap the error and return it as the first item of the stream
                    final_stream = Some(futures::stream::once(async { Err(e) }).chain(s).boxed());
                    break;
                }
                Ok(Some(Ok(item))) => {
                    // Success! Re-assemble the stream by putting the first item back
                    final_stream = Some(futures::stream::once(async { Ok(item) }).chain(s).boxed());
                    break;
                }
                Ok(None) => {
                    final_stream = Some(s.boxed());
                    break;
                }
                Err(_) => {
                    tracing::error!(
                        "LLM first token timeout (Attempt {}/{}). Rotating model.",
                        attempts,
                        max_attempts
                    );
                    self.nim_client.rotate_model();
                    if attempts == max_attempts {
                        return Err(AgentError::Llm("LLM stream connection timed out after multiple attempts. We've rotated the model; please try your request again.".to_string()));
                    }
                }
            }
        }

        let stream = final_stream.ok_or_else(|| {
            AgentError::Llm("Failed to establish neural stream after multiple attempts.".into())
        })?;

        // --- Tactical Mid-Turn Persistence ---
        // We set up a channel to receive tool results from bridges as they happen.
        let (htx, mut hrx) = mpsc::unbounded_channel::<HistoryEvent>();
        let msgs = self.messages.clone();

        // Background listener for history events
        tokio::spawn(async move {
            while let Some(event) = hrx.recv().await {
                match event {
                    HistoryEvent::ToolResult { name, content } => {
        ...
        let mut has_yielded_content = false;
        let mapped_stream = stream.map(move |res| match res {
            ...
            Ok(MultiTurnStreamItem::StreamAssistantItem(StreamedAssistantContent::Reasoning(
                ...
                has_yielded_content = true;
                Ok(format!("<thought>{}</thought>", reasoning.display_text()))
            }
            ...
            Ok(MultiTurnStreamItem::StreamAssistantItem(
                ...
                rig::streaming::ToolCallDeltaContent::Delta(d) => Ok(d),
                _ => Ok(String::new()),
            },
            Ok(_) => Ok(String::new()),
