...
pub async fn run_scan(
    ...
    // If pipeline mode is enabled, run the 8-position pipeline
    if config.pipeline.enabled {
        ui::print_status("PIPELINE", "8-Position Sovereign Architecture ACTIVE");

        // Initialize MCP and dependencies manually for pipeline context
        let memory = std::sync::Arc::new(crate::memory::qdrant::InMemoryStore::from_config(config));
        let generator: std::sync::Arc<dyn crate::memory::embeddings::EmbeddingGenerator> =
            std::sync::Arc::new(crate::memory::embeddings::FallbackGenerator::new(
                config.memory.vector_size as usize,
            ));
        let recon_graph = std::sync::Arc::new(tokio::sync::Mutex::new(
            crate::core::recon_graph::ReconGraph::new(target, config.agent.max_iterations),
        ));
        ...
        .await;
        let context = std::sync::Arc::new(crate::core::pipeline::PipelineContext::new(
            target.to_string(),
            recon_graph,
            memory,
            generator,
        ...
        // NIM is not created in cli but inside agent or pipeline. Let's create it.
        let nim_client = std::sync::Arc::new(crate::llm::NimClient::from_config(config).unwrap());

        let mut pipeline =
            crate::core::pipeline::MissionPipeline::new(config.clone(), nim_client, context)
                .await
                .map_err(|e| color_eyre::eyre::eyre!("Pipeline init failed: {}", e))?;

        pipeline.set_mcp_server(mcp_server);

        let result = pipeline
            .run("Commence full reconnaissance mission.")
            .await
            .map_err(|e| color_eyre::eyre::eyre!("Pipeline execution failed: {}", e))?;

        if result.success {
            ui::print_status("PIPELINE", "Mission Completed Successfully");
            if let Some(report) = result.report {
                println!("\n{}\n", report);
            }
        } else {
            ui::print_status("PIPELINE", "Mission Aborted or Failed");
            if let Some(err) = result.report {
                println!("Reason: {}", err);
            }
        }
    ...
    // Initialize classic single agent
    let mut agent = core::agent::ReconAgent::new(config.clone())
        .await
        .map_err(|e| color_eyre::eyre::eyre!("Agent init failed: {}", e))?;
