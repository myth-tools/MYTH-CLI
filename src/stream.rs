...
pub async fn consume_agent_stream(
    ...
    let mut current_timeout = 120; // 120s max for the VERY first cold start
    let mut active_tools = 0;

    loop {
        let timeout_duration = if active_tools > 0 {
            // While a tool is running, we allow up to 10 minutes (600s) for it to complete.
            std::time::Duration::from_secs(600)
        } else {
            // If no tools are running, it's just the network LLM connection.
            std::time::Duration::from_secs(current_timeout)
        };

        let chunk_result = tokio::select! {
            res = tokio::time::timeout(timeout_duration, stream.next()) => match res {
                Ok(res) => res,
                Err(_) => {
                    let err_msg = if active_tools > 0 {
                        "✗ Tool execution stalled: No response from tool sandbox within 600s."
                    } else {
                        "✗ Neural stream degraded: No response from operative hub within network limits."
                    };
                    writeln!(writer, "\n{}", err_msg.bright_red()).ok();

                    // Recover from silent 429 rate-limit hang in reqwest-eventsource
                    writeln!(writer, "  {} Detected silent rate-limit or endpoint stall.", "⚡".yellow()).ok();
                    if agent.nim_client().client_count() > 1 {
                        agent.nim_client().rotate_key();
                        writeln!(writer, "  {} Initiated tactical key rotation. Please retry your request.", "🔄".cyan()).ok();
                    } else {
                        agent.nim_client().rotate_model();
                        writeln!(writer, "  {} Initiated tactical model fallback. Please retry your request.", "🔄".cyan()).ok();
                    }
            ...
            Some(event) = event_rx.recv() => {
                match event {
                    ...
                    crate::tui::app::TuiEvent::ToolFinished { tool: _, success: _ } => {
                        if active_tools > 0 { active_tools -= 1; }
                        current_timeout = 30; // Reset network timeout to 30s after tool finishes since LLM is warm
        ...
        match chunk_result {
            Ok(chunk) => {
                // If we get any token, the LLM is awake. Future idle gaps shouldn't exceed 30s
                current_timeout = 30;
