...
/// Sensitive host paths that should never be referenced in arguments.
const SENSITIVE_PATHS: &[&str] = &[
    ...
    "/dev/vda",
    "/dev/nvme",
    "/mnt", // except our workspace — handled separately
    "/media",
    "/.ssh",
    "/id_rsa",
    "/id_ed25519",
            ...
            if OUTPUT_FLAGS.contains(arg) {
                ...
                if let Some(next_arg) = args.get(i + 1) {
                        ...
                        && !next_arg.starts_with("./")
                    {
                        ...
                        return Err(format!(
                            "🚫 Cannot write to '{}'. Only /workspace and /tmp are writable.",
                            next_arg
                        ));
                    }
                }
