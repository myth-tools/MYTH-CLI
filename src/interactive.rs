...
pub async fn run_interactive(config: &config::AppConfig) -> Result<()> {
    ...
    // Parallel Tool Discovery: Populate autocompletion cache without blocking the UI
    let disc_tools_clone = discovered_tools.clone();
    let mcp_clone = agent.mcp_server().discovery();
    tokio::spawn(async move {
        let disc = mcp_clone.read().await;
        let mut tools = disc_tools_clone.lock().unwrap();
