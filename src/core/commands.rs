...
fn levenshtein_distance(s1: &str, s2: &str) -> usize {
    let s1_indices: Vec<char> = s1.chars().collect();
    let s2_indices: Vec<char> = s2.chars().collect();
    let m = s1_indices.len();
    let n = s2_indices.len();
    ...
    for i in 1..=m {
        for j in 1..=n {
            if s1_indices[i - 1] == s2_indices[j - 1] {
                dp[i][j] = dp[i - 1][j - 1];
            } else {
                dp[i][j] =
...
pub fn tokenize_semantics(input: &str) -> Vec<SemanticToken> {
    ...
    // If the first token isn't a valid command, treat everything as "Other" (no highlighting)
...
pub fn get_argument_suggestions(input: &str, ctx: &CommandContext) -> Vec<String> {
    ...
    .to_lowercase();
    let current_token = tokens.last().cloned().unwrap_or("");

    let mut suggestions = Vec::new();

    // Profile Suggestions
    if (verb == "scan" && tokens.contains(&"--profile")) || verb == "profile" {
        let partial = if tokens.last() == Some(&"--profile") {
            ""
        } else {
            current_token
        };
    ...
    // Target Suggestions
    if verb == "target"
        || (verb == "scan" && tokens.len() == (if input.starts_with('/') { 2 } else { 1 }))
    {
        let partial = tokens.last().cloned().unwrap_or("");
        for target in ctx.mission_targets {
            if target.starts_with(partial) && target != partial {
...
pub fn get_ghost_suggestion(input: &str, ctx: &CommandContext) -> Option<String> {
    ...
    // If it's just one token, suggest command verbs
...
pub async fn handle_command(
    ...
    match verb_raw.as_str() {
        ...
        "master" | "ultra" if is_slash || is_legacy => {
             if args.is_empty() {
                ...
                let mut tor_fallback_override = true; // Default to true for mission verbs
    ...
    if let Some(action) = action_opt {
        ...
        // Unknown slash command -> block and suggest with premium aesthetic
        let mut msg = format!(
            "\n{} {} Command '{}' not recognized by tactical matrix.\n",
            "✗".bright_red(),
            "ACCESS DENIED //".bold(),
            verb_raw.bold()
        );
        if let Some(suggestion) = find_smart_suggestion(&verb_raw) {
            ...
            msg.push_str(&format!(
                "     {} Press {} to execute or {} for catalog.\n",
                CyberTheme::dim("┃"),
                CyberTheme::primary("ENTER").bold(),
                "/help".bright_white()
            ));
        } else {
            msg.push_str("   Type '/help' to catalog all available mission assets.\n");
        }
...
"#;
    ...
    async fn test_argument_suggestions() {
        ...
        let ctx = CommandContext {
            config: agent.config(),
            mission_targets: &vec!["google.com".to_string()],
        };

        // Test profile suggest (assuming 'stealth' is in agent.yaml)
        let suggestions = get_argument_suggestions("/scan google.com --profile stea", &ctx);
        assert!(suggestions.contains(&"stealth".to_string()));
