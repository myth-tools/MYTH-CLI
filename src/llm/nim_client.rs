...
use crate::config::{AgentSlotConfig, AppConfig};
...
impl NimClient {
    ...
    /// Get a Rig AgentBuilder pre-configured for a specific pipeline slot.
    /// This allows each position in the pipeline to override the model, temperature,
    /// and max_tokens dynamically while sharing the same underlying key pool.
    pub fn agent_builder_for_slot(&self, slot: &AgentSlotConfig) -> AgentBuilder<CompletionModel> {
        let client = self.client();
        client
            .agent(slot.model.as_deref().unwrap_or(&self.primary_model))
            .temperature(slot.temperature as f64)
            .max_tokens(slot.max_tokens as u64)
    }
