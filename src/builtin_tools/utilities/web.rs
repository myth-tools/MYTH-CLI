...
impl WebAutomation {
    pub async fn new(config: WebConfig) -> Result<Self, WebError> {
        ...
        let browser = if config.headless_browser {
            match Self::init_headless_browser(&config).await {
                Ok(b) => Some(b),
                Err(e) => {
                    eprintln!("Warning: Failed to initialize headless browser: {}", e);
                    None
                }
            }
        } else {
    ...
    async fn init_headless_browser(config: &WebConfig) -> Result<Browser, WebError> {
        ...
        if let Some(binary_path) = &config.browser_binary {
            options.path = Some(PathBuf::from(binary_path));
        }

        Browser::new(options).map_err(|e| WebError::BrowserError(e.to_string()))
    }
