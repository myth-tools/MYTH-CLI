...
pub async fn boot_sequence(config: &crate::config::AppConfig) {
    ...
    // 2. Neural Uplink Connectivity Audit (Live Diagnostic)
    let health_status = crate::llm::health::perform_health_check(config).await;
    let uplink_audit = || async {
        match health_status {
            crate::llm::health::HealthStatus::Ok => {
                CyberTheme::secondary("STABLE").bold().to_string()
            }
            crate::llm::health::HealthStatus::NoKeys => {
                CyberTheme::accent("DETACHED").bold().to_string()
            }
            _ => CyberTheme::warning("UNSTABLE").bold().to_string(),
        }
    };
    ...
    let pipeline_status = if config.pipeline.enabled {
        CyberTheme::secondary("ACTIVE").bold().to_string()
    } else {
        CyberTheme::dim("DORMANT").to_string()
    };
    ...
    let checks = [
        ...
        ("SOVEREIGN PIPELINE ARCH", pipeline_status, true),
    ...
    // Detailed Neural Link Report (Mission Critical)
    crate::llm::health::display_health_report(&health_status);

    if !matches!(health_status, crate::llm::health::HealthStatus::Ok) {
        println!(
            "\n  {} {}",
            "⚠".yellow().bold(),
            "Link instability detected. Review the report above.".yellow()
        );
        print!(
            "  {} Press {} to continue mission, or {} to abort: ",
            "◈".cyan(),
...
pub fn print_mission_header(config: &crate::config::AppConfig) {
    ...
    let pipe_status = if config.pipeline.enabled {
        "[ SOVEREIGN ]"
    } else {
        "[ LINEAR ]"
    };
    let (p_content, p_len) = format_telemetry("PIPELINE", ">>", pipe_status, false);
    print_row(p_content, p_len);
